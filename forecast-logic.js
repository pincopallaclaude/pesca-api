...
const { fetchStormglassData } = require('./services/stormglass.service.js');
...
const { runProactiveAnalysis } = require('./services/proactive_analysis.service.js');
...
function getMostSignificantWeatherCode(hourlyWmoCodes) {
    // Priorità decrescente (i codici più alti sono generalmente più severi)
    const priorityOrder = [
        95, 96, 99, // Temporali
        71, 73, 75, 77, 85, 86, // Neve
        51, 53, 55, 56, 57, 61, 63, 65, 66, 67, 80, 81, 82, // Pioggia/Rovesci
        3, // Nuvoloso
    ...
    return hourlyWmoCodes[12] || hourlyWmoCodes[0]; // Fallback sulle 12:00 o sul primo disponibile
}
...
// [MODIFICATO] Aggiunto console.time e timeout di axios
async function fetchOpenMeteoHourly(lat, lon) {
    ...
    // Aggiungiamo un timeout di 15 secondi per evitare attese infinite
...
async function fetchWwoDaily(lat, lon) {
    ...
    try {
        // Aggiungiamo un timeout di 15 secondi
...
async function getUnifiedForecastData(location) {
    ...
    try {
        ...
        // [MODIFICATO] Aggiunto console.time per l'aggregazione delle chiamate API
        ...
        console.timeEnd("PERF: Promise.all(API_Calls)"); // Fine timer

        // [DEBUG LOG RESTORED] Summarizes the status of the fetched API data.
        console.log(`\n******************************************************`);
        ...
        for (const wwoDay of wwoDailyData) {
            ...
            const hourlyScoresData = omHourly.map(h => {
                ...
                if (!hasLoggedScoreParams) {
                    const firstWwoDate = wwoDailyData[0].date;
                    const firstOmHour = parseInt(omHourly[0].time.split(':')[0], 10);

                    console.log(`[ShouldLog DEBUG] Hour: ${currentHour} | isFirstDay?: ${date} === ${firstWwoDate} (${date === firstWwoDate}) | isFirstHour?: ${currentHour} === ${firstOmHour} (${currentHour === firstOmHour})`);
                }
            ...
            // 1. Estrai tutti i codici meteo orari del giorno
            const allHourlyWmoCodes = omHourly.map(h => h.weatherCode);
            // 2. Determina il codice più significativo per l'intera giornata
            const mostSignificantWmoCode = getMostSignificantWeatherCode(allHourlyWmoCodes);

            // Dati che prima erano basati su un'ora rappresentativa, ora usano il codice significativo
            const dailyWeatherCode = converter.convertWmoToWwoCode(mostSignificantWmoCode);
            const dailyMeteoIcon = formatter.getMeteoIconFromCode(mostSignificantWmoCode);

            // Usa un dato fisso per la direzione del vento giornaliera (es. media o dalle 14:00)
            ...
            // 'representativeHourData' viene SEMPRE usato per i dati aggregati della card giornaliera.
            const representativeHourData = enrichedHourlyData.find(h => h.time.startsWith('14:')) ?? enrichedHourlyData[12];

            // 'currentHourData' viene usato SOLO per il primo giorno, per mostrare i dati "live".
            const isToday = (dayIndex === 0); // Assumendo che dayIndex sia definito prima del loop
            const liveHourData = isToday 
                ? (enrichedHourlyData.find(h => h.hour >= new Date().getHours()) ?? enrichedHourlyData[0])
                : representativeHourData;

            const temperaturaAcqua = String(Math.round(omHourly.map(h => h.waterTemperature).reduce((a, b) => a + b, 0) / omHourly.length));
            
            // Dati della corrente, presi dall'ora rilevante (live per oggi, rappresentativa per i giorni futuri)
            const { currentSpeedKn, currentDirectionStr } = liveHourData || {};
            const currentSpeedFallback = currentSpeedKn || 'N/D';
            const currentDirectionFallback = currentDirectionStr || 'N/D';
            const currentDataString = (currentSpeedFallback !== 'N/D' && currentDirectionFallback !== 'N/D') 
                ? `${parseFloat(currentSpeedFallback).toFixed(1)} kn ${currentDirectionFallback}` 
                : 'N/D';

            finalForecast.push({
                giornoNome: formatter.capitalize(format(parseISO(date), 'eee', { locale: it })),
                giornoData: format(parseISO(date), 'dd/MM'),
                meteoIcon: formatter.getMeteoIconFromCode(liveHourData.weatherCode), // Icona "live" per oggi, rappresentativa per domani
                ...
                // Usa SEMPRE l'ora rappresentativa per la direzione del vento giornaliera, per coerenza
                ventoDati: `${(Math.max(...omHourly.map(h => h.windSpeed)) / 1.852).toFixed(0)} kn ${converter.degreesTo16PointDirection(representativeHourData.windDirection)}`,
                // Usa l'ora live per il mare del giorno corrente, e quella rappresentativa per i giorni futuri
        ...
        // [MODIFICATO] Refactoring del trigger proattivo: uso di IIFE async non-bloccante
        // [PHANTOM TRIGGER] Avvia l'analisi AI in background senza bloccare la risposta.
        (async () => {
            try {
