...
function calculateHourlyPescaScore(params) {
    ...
    const {
        ...
        isNewOrFullMoon, cloudCover, waveHeight, waterTemp, currentVelocity
    } = params;
    ...
    if (currentVelocity !== null) {
        if (currentVelocity > 1) { 
            score -= 1.0; 
            reasons.push({ icon: 'currents', text: "Correnti troppo forti (>1 m/s)", points: "-1.0", type: "negative" }); 
        } else {
            reasons.push({ icon: 'currents', text: "Correnti neutre", points: "+0.0", type: "neutral" });
        }
    } else {
