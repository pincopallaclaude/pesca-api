...
async function calculateHourlyPescaScoreWithML(params, astronomyData, location, shouldLog = false) {
    ...
    if (mlPrediction.predicted !== null && mlPrediction.confidence > 0.6) {
        ...
        if (shouldLog) {
            logger.log(`[Score ML] Blending: ML(${mlPrediction.predicted.toFixed(2)}) * 0.8 + Rules(${ruleBasedScore.toFixed(2)}) * 0.2 = ${finalScore.toFixed(2)}`);
        }
        ...
        if (shouldLog) {
            logger.log(`[Score ML] Low confidence blend: ML(${mlPrediction.predicted.toFixed(2)}) * 0.5 + Rules(${ruleBasedScore.toFixed(2)}) * 0.5 = ${finalScore.toFixed(2)}`);
        }
    }
