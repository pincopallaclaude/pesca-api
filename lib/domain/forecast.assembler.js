...
export function assembleForecastData(wwoDailyData, openMeteoHourlyData, stormglassData, resolvedName) {
    ...
    for (const wwoDay of wwoDailyData) {
        ...
        // NUOVO: Aggiungiamo un campo separato per la descrizione testuale del mare
        const seaStateDescription = formatter.getSeaStateDescription(liveHourData.waveHeight);

        finalForecast.push({
            location: { name: resolvedName }, // Aggiunto il nome della località risolto
            seaStateDescription: seaStateDescription, // <-- CAMPO AGGIUNTO
