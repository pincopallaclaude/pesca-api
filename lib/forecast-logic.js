...
async function fetchAndProcessForecast(location) {
    const cacheKey = `forecast-data-final-v3-${location}`; // Invalida di nuovo la cache
    ...
    try {
        ...
        for (const wwoDay of wwoDailyData) {
            ...
            const hourlyScores = omHourly.map(h => { /* ... logica score ... */ return { hour: parseInt(h.time.split(':')[0],10), score: 3.0 }; }); // Semplificato per debug
            ...
            finalForecast.push({
                ...
                meteoIcon: getMeteoIconFromCode(omHourly[new Date().getHours()]?.weatherCode ?? omHourly[0].weatherCode),
        ...
        console.error("[FATAL ERROR in fetchAndProcessForecast]:", error.message, error.stack);
        // NON FAR CRASHARE IL SERVER. Restituisci una risposta vuota ma valida.
        return {
            fonti: "Error",
            forecast: [],
            dateRange: "N/A"
