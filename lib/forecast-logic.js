...
async function fetchAndProcessForecast(location) {
    const cacheKey = `forecast-data-v-FINALISSIMA-${location}`;
    ...
    try {
        ...
        for (const wwoDay of wwoDailyData) {
            ...
            const hourlyClientFormat = omHourly.map(h => ({
                time: h.time, tempC: String(Math.round(h.temperature)),
            ...
            // --- NUOVA LOGICA PER AGGREGARE LE "RAGIONI" PIÙ IMPORTANTI ---
            const representativeHour = omHourly.find(h => h.time === '12:00') || omHourly[0];
            const representativeScoreData = calculateHourlyPescaScore({
                pressure: representativeHour.pressure,
                trendPressione,
                windSpeedKph: representativeHour.windSpeed,
                isNewOrFullMoon,
                cloudCover: representativeHour.cloudCover,
                waveHeight: representativeHour.waveHeight,
                waterTemp: representativeHour.waterTemperature,
            });
            ...
            finalForecast.push({
                ...
                pescaScoreData: {
                    ...
                    reasons: representativeScoreData.reasons ?? []
                },
