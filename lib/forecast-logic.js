    ...
    try {
        ...
        for (const wwoDay of wwoDailyData) {
            ...
            const hourlyClientFormat = omHourly.map(h => {
                ...
                // Associamo ad ogni ora la marea più vicina
                const closestTide = [...highTides, ...lowTides].reduce(
                    (prev, curr) => {
                        const prevDiff = Math.abs(timeToHours(prev.tideTime) - currentHour);
                        const currDiff = Math.abs(timeToHours(curr.tideTime) - currentHour);
                        return currDiff < prevDiff ? curr : prev;
                    }, 
                    { tideTime: "00:00", tide_type: 'LOW' } // Valore iniziale
                );
                ...
                return {
                    ...
                    // ====> NUOVI DATI ORARI PER IL PANNELLO ESPANDIBILE
                    windSpeedKn: Math.round(h.windSpeed / 1.852),
                    windDirectionDegrees: h.windDirection,
                    waveHeight: h.waveHeight?.toFixed(1) ?? 'N/D',
                    precipitationProbability: h.precipitationProbability ?? 0,
                    precipitation: h.precipitation?.toFixed(1) ?? 'N/D',
                    pressure: Math.round(h.pressure),
                    humidity: h.humidity,
                    tide: `${closestTide.tide_type === 'HIGH' ? 'Alta' : 'Bassa'} ${formatTimeToHHMM(closestTide.tideTime)}`
                    // La velocità della corrente verrà aggiunta dinamicamente per Posillipo in un secondo momento
                };
            });
...
// --- NUOVO CODICE DA INCOLLARE (IN SOSTITUZIONE FINO ALLA FINE DEL push) ---
            ...
            finalForecast.push({
                ...
                // Ora usiamo la variabile `currentSpeedKn` che può contenere 'N/D' o il valore di Stormglass
