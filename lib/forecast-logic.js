...
const convertWmoToWwoCode = (wmoCode) => {
    ...
    if (code >= 1 && code <= 3) return '116';
    if (code >= 45 && code <= 48) return '260';
    if (code >= 51 && code <= 65) return '296';
    if (code >= 66 && code <= 67) return '314';
    if (code >= 71 && code <= 77) return '329';
    if (code >= 80 && code <= 82) return '353';
    if (code >= 95 && code <= 99) return '389';
    return '119';
};
...
const getMeteoIconFromCode = (code) => { // LA FUNZIONE MANCANTE
...
async function fetchAndProcessForecast(location) {
    const cacheKey = `forecast-data-v2.0-${location}`; // Nuova versione per invalidare
    ...
    try {
        ...
        for (const wwoDay of wwoDailyData) {
            ...
            const hourlyScores = omHourly.map(h => {
                ...
                return { hour, score: scoreData.numericScore };
            });
            ...
            const maxTemp = Math.max(...omHourly.map(h => h.temperature));
            const minTemp = Math.min(...omHourly.map(h => h.temperature));
            const avgTemp = omHourly.map(h => h.temperature).reduce((a,b)=>a+b,0) / omHourly.length;
            const avgHumidity = omHourly.map(h => h.humidity).reduce((a,b)=>a+b,0) / omHourly.length;
            const maxWind = Math.max(...omHourly.map(h=>h.windSpeed));
            const avgWaterTemp = omHourly.map(h=>h.waterTemperature).reduce((a,b)=>a+b,0) / omHourly.length;
            ...
            finalForecast.push({
                ...
                temperaturaAvg: String(Math.round(avgTemp)),
                pressione: String(Math.round(dailyPressure)),
                umidita: String(Math.round(avgHumidity)),
                trendPressione,
                ventoDati: `${(maxWind / 1.852).toFixed(0)} kn ${degreesTo16PointDirection(currentHourData.windDirection)}`,
                pescaScoreData: {
                    numericScore: avgNumericScore,
                    displayScore: displayScore,
                    hourlyScores: hourlyScores.map(h => ({ time: `${String(h.hour).padStart(2, '0')}:00`, score: h.score })),
                },
                pescaScore: displayScore, // Per retrocompatibilità se serve
                ...
                temperaturaAcqua: String(Math.round(avgWaterTemp)),
                velocitaCorrente: 'N/D', // Dato non più disponibile con la nuova strategia
                hourly: hourlyClientFormat,
            });

            previousDayData = { dailyPressure: dailyPressure };
        }
