...
function findBestTimeWindow(hourlyScores, startHour, endHour) {
    ...
    return `${String(bestWindowStart).padStart(2, '0')}:00 - ${String(bestWindowStart + 2).padStart(2, '0')}:00`;
}
        ...
        for (const wwoDay of wwoDailyData) {
            ...
            const findClosestTide = (hour, tides) => {
                ...
                return tides.reduce((prev, curr) => {
                    const prevTimeHours = formatter.timeToHours(prev.tideTime);
                    const currTimeHours = formatter.timeToHours(curr.tideTime);
            ...
            const hourlyClientFormat = enrichedHourlyData.map(h => {
                ...
                return {
                    ...
                    windDirectionDegrees: h.windDirection, // Passiamo i gradi, la conversione avviene dopo
                    pressure: h.pressure,
                    humidity: h.humidity,
                    waveHeight: h.waveHeight,
