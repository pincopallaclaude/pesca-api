...
function calculateHourlyPescaScore(params) {
    ...
    if (trendPressione === '↓') score += 1.5; else if (trendPressione === '↑') score -= 1.0;
    if (windSpeedKph > 5 && windSpeedKph < 20) score += 1.0; else if (windSpeedKph > 30) score -= 2.0;
    if (isNewOrFullMoon) score += 1.0;
    if (cloudCover > 60) score += 1.0; else if (cloudCover < 20 && pressure > 1018) score -= 1.0;
    if (waveHeight !== null) {
...
async function fetchAndProcessForecast(location) {
    const cacheKey = `forecast-data-v-ULTIMA-PROVA-${location}`; // Cache invalidation aggressiva
