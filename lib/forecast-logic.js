...
const getStatusLabel = (value) => {
    ...
    if (typeof value === 'string' && (value.trim().toUpperCase() === 'N/D' || value.trim() === '' || value.trim() === '→')) {
        return 'N/D'; // Consideriamo 'N/D' anche per il trend '→' se vogliamo solo loggare un cambiamento effettivo (↓/↑) come 'SI'
    }
...
function calculateHourlyPescaScore(params, shouldLog = false) {
    ...
    if (shouldLog) { // <--- Il log viene stampato SOLO se shouldLog è true (prima ora)
        ...
        const trendStatus = (trendPressione === '↓' || trendPressione === '↑') ? 'SI' : 'N/D'; // Pressione SI, Trend SI solo se è in calo/aumento
        ...
        const currentStatus = getStatusLabel(currentSpeedKn);
        const currentDirStatus = getStatusLabel(currentDirectionStr);

        // Stampa il riepilogo richiesto, senza l'ora specifica e il separatore
        console.log(`\n [Score Calc DEBUG] Parametri Ricevuti per il calcolo (Check di disponibilità):`);
...
async function getUnifiedForecastData(location) {
    ...
    try {
        ...
        // ==========================================================
        // ✨ NUOVO DEBUG: Riepilogo Stato API (Reintrodotto)
        // ==========================================================
        let stormglassStatus = 'N/D (Non Richiesto)';
        ...
        // Stato Stormglass (solo se richiesto, cioè se la location è Posillipo)
        if (isPosillipo) {
            if (stormglassData) {
                stormglassStatus = 'OK';
            } else {
                // Se fetchStormglassData è fallita, il suo catch ha restituito null
                // Si usa un messaggio generico ma si incoraggia a controllare il log specifico del catch
                stormglassStatus = 'ERROR: Controllare il log (e.g., status code 402)';
                criticalDataWarning = 'Stormglass non ha fornito i dati Corrente/Acqua.';
            }
        }

        console.log(`\n\n[DEBUG Summary] --- Stato API e Dati Critici ---`);
        console.log(`[DEBUG Summary] 1. WWO Status (Meteo Base/Maree): ${wwoStatus}`);
        console.log(`[DEBUG Summary] 2. Open Meteo Status (Vento/Onde/Temp Acqua): ${omStatus}`);
        console.log(`[DEBUG Summary] 3. StormGlass Status (Correnti): ${stormglassStatus}`);
        console.log(`[DEBUG Summary] ----------------------------------`);
        
        if (criticalDataWarning) {
            console.warn(`\n[DEBUG Summary] DATI CRITICI ASSENTI: ${criticalDataWarning}`);
        }
        // ==========================================================
        ...
        for (const wwoDay of wwoDailyData) {
            ...
            const hourlyScoresData = omHourly.map(h => {
                ...
                const shouldLog = !hasLoggedScoreParams && date === wwoDailyData[0].date && currentHour === omHourly[0].hour;
