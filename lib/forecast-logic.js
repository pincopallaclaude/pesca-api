...
// --- NUOVO CODICE INCOLLATO INIZIO ---
                ...
                if (isPosillipo && stormglassData && stormglassData[date]) {
                    ...
                    // Cerchiamo la corrispondenza oraria.
                    // Nota: stormglassData[date] è un array di oggetti, es. [{ hour: '00', ... }, { hour: '01', ... }]
                    ...
                    if (sgHourData) {
                        ...
                        // Controlliamo se la velocità della corrente è un numero valido
...
// --- NUOVO CODICE INCOLLATO FINE ---

            const currentHourData = omHourly.find(h => parseInt(h.time.split(':')[0]) >= new Date().getHours()) ?? omHourly[0];

            // Rimuovi la sezione DEBUG estremo precedente che è stata spostata in `hourlyClientFormat`
            // let currentSpeedKn = 'N/D';
            // console.log(`[DEBUG] Giorno: ${date}, isPosillipo: ${isPosillipo}`);
            // ... (altri console.log e logica di assegnazione rimossi) ...
