...
async function fetchAndProcessForecast(location) {
    ...
    try {
        ...
        // (Vedi dal punto 'let previousDayData = null;' in avanti)
        // ******************************************************************************
        ...
        const getStormglassCurrent = (date, hour) => {
            if (!isPosillipo || !stormglassData || !stormglassData[date]) return 'N/D';
            const hourStr = String(hour).padStart(2, '0');
            const sgHourData = stormglassData[date].find(sg_h => sg_h.hour === hourStr);
            return sgHourData ? sgHourData.currentSpeedKn : 'N/D';
        };
        ...
        for (const wwoDay of wwoDailyData) {
            ...
            const enrichedHourlyData = omHourly.map(h => {
                ...
                const currentSpeedKn = getStormglassCurrent(date, currentHour);
            ...
            const hourlyClientFormat = enrichedHourlyData.map(h => {
                ...
                // Nota: i log di debug per la corrente sono ora nella sezione di "Data Merge" sopra.
        ...
        // 4. Se c'è un dato obsoleto, lo restituiamo come fallback (Stale-While-Error)
        // Poiché myCache non supporta l'ottenimento di dati scaduti, proviamo a ottenere il dato
        // subito prima di lanciare l'errore se non è la prima volta (limitazione NodeCache standard).
        const cachedData = myCache.get(cacheKey);
        
        // Rilascio del LOCK con errore
        if (cacheLocks.has(cacheKey)) {
             cacheLocks.get(cacheKey).catch(() => {}); // Previene UnhandledRejection in caso di errore
             cacheLocks.delete(cacheKey);
             rejectLock(error);
        }
        
        // Se si è verificato un errore, ma esiste un dato fresco/vecchio, lo restituiamo, altrimenti l'errore.
        if (cachedData) {
        ...
        if (cacheLocks.has(cacheKey)) {
            // Nota: Se `finally` viene eseguito dopo il `resolveLock` o `rejectLock`, `cacheLocks.get(cacheKey)` 
            // contiene la Promise già risolta/rigettata, ma la rimozione è sicura.
            cacheLocks.delete(cacheKey);
        }
    }
}
