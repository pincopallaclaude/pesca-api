...
async function fetchStandardData(lat, lon) {
    ...
    const processedForecast = forecastData.map(dailyData => {
        ...
        // Modifica 1: Aggiunto log di debug
        console.log(`[DEBUG] Processando dati per il giorno: ${dateString}`);
        ...
        // Logghiamo solo il primo elemento per non inondare la console
        if (hourlyTransformed.length > 0) {
            console.log('[DEBUG] Dati orari trasformati (primo elemento):', hourlyTransformed[0]);
        }
        console.log(`[DEBUG] Numero totale di ore trasformate: ${hourlyTransformed.length}`);
...
async function fetchAndProcessForecast(location) {
    ...
    if (location === POSILLIPO_COORDS) {
        ...
        try {
            // Tentativo di fetch e processamento dei dati da Stormglass e WWO
            ...
            // Se la risposta di WWO è vuota, consideralo un fallimento e vai al fallback
            if (!wwoAstroTideData || wwoAstroTideData.length === 0) {
                throw new Error("WWO data for Stormglass processing is empty.");
            }

            const stormglassData = await fetchStormglassData(lat, lon);

            // Tutta la logica di processamento, che è la fonte del crash,
            // ora è al sicuro all'interno del blocco try.
            ...
            // Se QUALSIASI cosa nel blocco try fallisce (chiamata di rete, parsing, logica),
            // il server non andrà in crash. Eseguirà questo blocco.
    ...
    for (const dailyData of rawForecastData.forecast) {
        ...
        for (let i = 0; i < rawForecastData.forecast.length; i++) {
            const dailyData = rawForecastData.forecast[i]; // L'oggetto con tutti i nostri dati calcolati
            // Prendiamo l'oggetto corrispondente dai dati RAW di WWO, che contiene winddir16Point
            const originalDailyData = wwoResponse.data.data.weather[i];

            let trendPressione = '→';
            if (previousDayData?.pressure) {
                if (dailyData.dailyPressure < previousDayData.pressure - 0.5) trendPressione = '↓';
                else if (dailyData.dailyPressure > previousDayData.pressure + 0.5) trendPressione = '↑';
            }
        }
        ...
        console.log(`[DEBUG VENTO] Ispezionando dailyData.hourly[4] per il giorno ${dailyData.date}:`, dailyData.hourly?.[4]);
        ...
        finalForecast.push({
            ...
            trendPressione,
            ventoDati: `${(dailyData.dailyWindSpeedKph / 1.852).toFixed(0)} kn ${originalDailyData.hourly[4]?.winddir16Point ?? ''}`.trim(),
    ...
    // Modifica 2: Aggiunto log di debug
    if (finalForecast.length > 0) {
        console.log('[DEBUG] Controllo finale. Primo giorno della previsione contiene la chiave "hourly":', finalForecast[0].hasOwnProperty('hourly'));
        if (finalForecast[0].hasOwnProperty('hourly')) {
            console.log(`[DEBUG] Numero di elementi nell'array hourly del primo giorno: ${finalForecast[0].hourly.length}`);
        }
    }
