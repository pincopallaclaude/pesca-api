...
// Assumi che fetchStormglassData usi una logica interna a stormglass.service.js
...
// --- SEZIONE UTILITIES (ESPORTATE) ---
/**
 * Converte i gradi in una direzione cardinale a 16 punti (N, NNE, NE...).
 * @param {number} deg - Gradi (0-360).
 * @returns {string} - Direzione cardinale.
 */
...
// Altre utilities qui se necessario...
const utilities = {
    capitalize,
    degreesTo16PointDirection, // Rende la funzione disponibile internamente ed esternamente
    areCoordsNear: (coords1, coords2, toleranceKm = 1) => {
...
// Devo importare le utility che userò in questo file nel contesto locale
const { areCoordsNear, degreesTo16PointDirection: degreesTo16PointDirectionLocal, capitalize: capitalizeLocal, formatTimeToHHMM, convertWmoToWwoCode, getSeaStateAcronym, getWeatherDescription, getStatusLabel, getMeteoIconFromCode } = utilities;
 ...
 * @returns {boolean}
 */
function areCoordsNearWrapper(coords1, coords2, toleranceKm = 1) {
    return utilities.areCoordsNear(coords1, coords2, toleranceKm);
}
...
async function getUnifiedForecastData(location) {
    ...
    try {
        ...
        if (isPosillipo) {
            ...
            // *** PASSAGGIO CRUCIALE: Passiamo le utility necessarie al servizio Stormglass
            promises.push(fetchStormglassData(lat, lon, utilities).catch(err => {
                // Logghiamo l'errore esatto per il debug
...
module.exports = { 
    ...
    myCache,
    // *** ESPORTAZIONE DELLE UTILITY PER I SERVICE ESTERNI (FIX) ***
    utilities 
};
