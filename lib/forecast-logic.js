...
function calculateHourlyPescaScore(params) {
    ...
    const { 
        hour, // Aggiunto per il log di debug
    ...
    // ==========================================================
    // ========= INIZIO LOG DEBUG PESCASCORE ====================
    // ==========================================================
    const waveHeightDisplay = waveHeight !== null && waveHeight !== undefined ? `${waveHeight.toFixed(2)} m` : 'N/D';
    const waterTempDisplay = waterTemp !== null && waterTemp !== undefined ? `${waterTemp.toFixed(1)} °C` : 'N/D';
    const currentSpeedDisplay = currentSpeedKn !== 'N/D' ? `${parseFloat(currentSpeedKn).toFixed(2)} kn` : 'N/D';
    const currentDirDisplay = currentDirectionStr !== 'N/D' ? currentDirectionStr : 'N/D';
    
    console.log(`\n[Score Calc DEBUG] Parametri ricevuti per l'ora ${String(hour).padStart(2, '0')}:00`);
    console.log(`  - Pressione/Trend: ${pressure.toFixed(1)} hPa / ${trendPressione}`);
    console.log(`  - Vento: ${windSpeedKph.toFixed(1)} km/h`);
    console.log(`  - Nuvolosità: ${cloudCover}%`);
    console.log(`  - Onde (WaveHeight): ${waveHeightDisplay}`);
    console.log(`  - Temp Acqua: ${waterTempDisplay}`);
    console.log(`  - Corrente (Speed/Dir): ${currentSpeedDisplay} / ${currentDirDisplay}`);
    console.log(`  - Luna (Fase Critica): ${isNewOrFullMoon ? 'SÌ' : 'NO'}`);
    // ==========================================================
    // ========= FINE LOG DEBUG PESCASCORE ======================
    // ==========================================================
    ...
    console.log(`[Score Calc DEBUG] Score Totale Finale: ${score.toFixed(1)}`);
...
async function fetchOpenMeteoHourly(lat, lon) {
    ...
    // Controllo basico sulla struttura dei dati
    if (!forecastResponse.data?.hourly || !marineResponse.data?.hourly) {
        throw new Error("Open Meteo data structure invalid or empty.");
    }
...
async function fetchWwoDaily(lat, lon) {
    ...
    if (!response.data?.data?.weather) throw new Error("WWO API response structure is invalid or missing 'weather'.");
    return response.data.data.weather;
}

// --- FUNZIONE PRINCIPALE UNIFICATA (Nuova Versione con Logging) ---
/**
...
async function getUnifiedForecastData(location) {
    ...
    // ==========================================================
    // ========= INIZIO LOGGING E STATO API =====================
    // ==========================================================
    console.log(`\n\n--- [MASTER FETCH] Inizio richiesta per location: "${location}" ---`);
    ...
    console.log(`[Master Fetch Log] Location normalizzata a 3 decimali: "${normalizedLocation}"`);
    ...
    // Stato per tracciare il successo/fallimento delle API
    let apiStatus = { 
        wwo: 'PENDING', 
        openMeteo: 'PENDING', 
        stormglass: isPosillipo ? 'PENDING' : 'SKIPPED' 
    };
    
    // ------------------- LOGICA DI CACHE LOCKING -------------------
    if (cacheLocks.has(cacheKey)) {
        console.log(`[Master Fetch Log] LOCK HIT: Chiave ${cacheKey} in fase di aggiornamento. Attendo...`);
    ...
    try {
        ...
        if (cachedData) {
            console.log(`[Master Fetch Log] Cache HIT for ${location}`);
            return cachedData; 
        }

        console.log(`[Master Fetch Log] Cache MISS for ${location}. Acquisizione LOCK e Fetching new data...`);
        ...
        // --- 1. PREPARAZIONE DELLE CHIAMATE CON CATCH E STATUS TRACKING ---
        const requests = [];

        // WWO Fetch (Meteo Base/Maree)
        requests.push(fetchWwoDaily(lat, lon).then(data => {
            apiStatus.wwo = 'OK';
            return data;
        }).catch(err => {
            apiStatus.wwo = `ERROR: ${err.message.substring(0, 50)}...`;
            console.error(`[API LOG] WWO: ERRORE: ${err.message}`);
            return null; // Ritorna null per non bloccare Promise.all
        }));

        // Open Meteo Fetch (Vento/Onde/Temp Acqua)
        requests.push(fetchOpenMeteoHourly(lat, lon).then(data => {
            apiStatus.openMeteo = 'OK';
            return data;
        }).catch(err => {
            apiStatus.openMeteo = `ERROR: ${err.message.substring(0, 50)}...`;
            console.error(`[API LOG] OpenMeteo: ERRORE: ${err.message}`);
            return null;
        }));

        // Stormglass Fetch (Correnti/Dati Avanzati)
        if (isPosillipo) {
            requests.push(fetchStormglassData(lat, lon).then(data => {
                if (!data) {
                    apiStatus.stormglass = 'N/D - Dati Vuoti';
                    return null;
                }
                apiStatus.stormglass = 'OK';
                return data;
            }).catch(err => {
                apiStatus.stormglass = `ERROR: ${err.message.substring(0, 50)}...`;
                console.error(`[API LOG] Stormglass: ERRORE: ${err.message}`);
                return null;
            }));
        } else {
             // Aggiungi un placeholder null per allineare l'array 'requests' con l'array 'results'
             requests.push(Promise.resolve(null));
        }

        // Recuperiamo i dati
        const [wwoDailyData, openMeteoHourlyData, stormglassData] = await Promise.all(requests);
        
        // --- CONTROLLO CRITICO DEI DATI BASE ---
        if (!wwoDailyData || !openMeteoHourlyData || wwoDailyData.length === 0 || Object.keys(openMeteoHourlyData).length === 0) {
             const errorMsg = "Impossibile procedere: Dati WWO o OpenMeteo mancanti o vuoti.";
             console.error(`[FATAL MERGE] ${errorMsg}`);
             // Imposta lo stato del lock come fallito e rilancia
        ...
        const getStormglassCurrent = (date, hour) => {
            ...
            // Utilizza stormglassData solo se è OK
            if (apiStatus.stormglass !== 'OK' || !stormglassData || !stormglassData[date]) return defaultData;
        ...
        for (const wwoDay of wwoDailyData) {
            ...
            const hourlyScoresData = omHourly.map(h => {
                ...
                // 2. Pass the correctly retrieved data to the score calculator.
                const scoreData = calculateHourlyPescaScore({
                    hour: currentHour, // Passiamo l'ora per il log di debug
                    pressure: h.pressure,
                    trendPressione,
        ...
        // ==========================================================
        // ========= LOG DI RIEPILOGO STATO API E DATI ==============
        // ==========================================================
        
        // Verifica se almeno un dato di corrente è stato recuperato (per 1 ora in 7 giorni)
        const hasCurrentDataOverall = finalForecast.some(day => 
            day.hourly.some(hour => hour.currentSpeedKn !== 'N/D')
        );

        console.log(`\n[DEBUG Summary] --- Stato API e Dati Critici ---`);
        console.log(`[DEBUG Summary] 1. WWO Status (Meteo Base/Maree): ${apiStatus.wwo}`);
        console.log(`[DEBUG Summary] 2. Open Meteo Status (Vento/Onde/Temp Acqua): ${apiStatus.openMeteo}`);
        console.log(`[DEBUG Summary] 3. StormGlass Status (Correnti): ${apiStatus.stormglass}`);

        if (apiStatus.wwo.startsWith('ERROR') || apiStatus.openMeteo.startsWith('ERROR')) {
            console.error(`[DEBUG Summary] FATALE: I dati meteo di base sono mancanti. Forecast incompleto/impossibile.`);
        }

        if (!isPosillipo) {
            console.log(`[DEBUG Summary] Corrente/Maree: SKIPPED (Location non è Posillipo).`);
        } else if (apiStatus.stormglass !== 'OK') {
            console.warn(`[DEBUG Summary] DATI CRITICI ASSENTI: Stormglass non ha fornito i dati Corrente/Acqua.`);
        } else if (!hasCurrentDataOverall) {
            console.warn(`[DEBUG Summary] ATTENZIONE: Stormglass OK, ma nessun dato di corrente (Corrente: N/D) è stato trovato nel forecast.`);
        } else {
            console.log(`[DEBUG Summary] Correnti/Maree: OK. (Dati recuperati in almeno un'ora).`);
        }
        ...
        console.log(`[Master Fetch Log] Cache aggiornata e LOCK rilasciato per: ${cacheKey}`);
        ...
        console.error(`[Master Fetch Log] Errore critico nel recupero dati per ${location}: ${error.message}`);
        // Rilascio il LOCK in caso di fallimento
        if (cacheLocks.has(cacheKey)) {
            cacheLocks.get(cacheKey).reject(error); // Reject la promise per i lock in attesa
            cacheLocks.delete(cacheKey); // Rimuovo il lock
        }
        throw error;
    } finally {
        // Rilascio il lock se presente e se il codice non è uscito tramite il catch (caso di successo)
        if (cacheLocks.has(cacheKey) && cacheLocks.get(cacheKey) instanceof Promise) {
            cacheLocks.delete(cacheKey);
        }
        console.log(`--- [MASTER FETCH] Fine richiesta per location: "${location}" ---`);
    }
}
...
module.exports = {
    ...
    areCoordsNear,
    calculateHourlyPescaScore // Esportata per eventuali test
};
