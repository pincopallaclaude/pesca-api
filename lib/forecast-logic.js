...
function calculateHourlyPescaScore(params) {
    ...
    // MODIFICA: Aggiunto currentDirectionStr ai parametri destrutturati per consistenza
    const { 
        pressure, trendPressione, windSpeedKph, isNewOrFullMoon, 
        cloudCover, waveHeight, waterTemp, currentSpeedKn,
        currentDirectionStr // <-- NUOVO: Aggiunto anche la direzione
    } = params;
    
    // ==========================================
    // ========= NUOVO LOG DI DEBUG =============
    // Log dettagliato dei valori in ingresso per il calcolo del punteggio.
    console.log(`[Score Calc DEBUG] Ora: ${params.hour}, Vento: ${windSpeedKph.toFixed(1)} km/h, Onde: ${waveHeight?.toFixed(2)} m, Corrente: ${currentSpeedKn} kn, Dir.Corrente: ${currentDirectionStr}`);
    // ==========================================
    ...
    // Nota: currentSpeedKn è una stringa ('N/D' o un numero.toFixed(1))
...
async function fetchStormglassData(lat, lon) {
    // MODIFICA CRITICA 1: Aggiungiamo 'currentDirection' ai parametri richiesti
    ...
    for (const hourData of response.data.hours) {
        ...
        // Valore Storm Glass (m/s) convertito in Nodi (Kn)
        let speedInKn = null; 
        let directionDeg = null; // NUOVO: Direzione in gradi
        
        if (hourData.currentSpeed && hourData.currentSpeed.sg != null && !isNaN(hourData.currentSpeed.sg)) {
            speedInKn = hourData.currentSpeed.sg * 1.94384; // m/s -> nodi
            // Correzione per valori molto piccoli, arrotondamento a 0.1 se > 0
            if (speedInKn > 0 && speedInKn < 0.1) speedInKn = 0.1;
        }

        // NUOVO: Estraiamo la direzione della corrente (in gradi)
        if (hourData.currentDirection && hourData.currentDirection.sg != null && !isNaN(hourData.currentDirection.sg)) {
            directionDeg = hourData.currentDirection.sg;
        }
        ...
        dataByDay[date].push({
            ...
            // NUOVO: Convertiamo i gradi in una direzione cardinale (es. 'NE') o 'N/D'
            currentDirectionStr: directionDeg != null ? degreesTo16PointDirection(directionDeg) : 'N/D',
        });
    }
    return dataByDay;
}
...
async function fetchAndProcessForecast(location) {
    ...
    try {
        ...
        // MODIFICA CRITICA 2: Funzione helper per ottenere i dati di Stormglass per una specifica ora (sia velocità che direzione)
        ...
        for (const wwoDay of wwoDailyData) {
            ...
            const enrichedHourlyData = omHourly.map(h => {
                ...
                // Estraiamo entrambi i dati, velocità e direzione
                const { currentSpeedKn, currentDirectionStr } = getStormglassCurrent(date, currentHour);
                
                // ==========================================================
                // ========= NUOVO LOG DI DEBUG PER L'UNIONE DEI DATTI =======
                // ==========================================================
                console.log(`[Data Merge DEBUG] Giorno ${date} @ ${h.time}: Corrente Stormglass -> ${currentSpeedKn} kn ${currentDirectionStr}`);
                // ==========================================================
                ...
                return { 
                    ...
                    currentHour, 
                    currentSpeedKn,
                    currentDirectionStr // <-- NUOVO: Aggiungiamo la direzione
                };
            });
            ...
            const hourlyScoresData = enrichedHourlyData.map(h => {
                const scoreData = calculateHourlyPescaScore({
                    ...
                    currentSpeedKn: h.currentSpeedKn, // <-- Passiamo la velocità unificata
                    currentDirectionStr: h.currentDirectionStr // <-- NUOVO: Passiamo la direzione unificata
                });
            ...
            const hourlyClientFormat = enrichedHourlyData.map(h => {
                ...
                return {
                    ...
                    currentDirectionStr: h.currentDirectionStr, // NUOVO: Direzione corrente
            ...
            const currentDirectionStr = currentHourData.currentDirectionStr; // NUOVO

            // LOGICA DI CORREZIONE REGRESSIONE (CRITICA)
            ...
            const currentDataString = 
                ...
                ? `${currentSpeedKn} kn ${currentDirectionStr}` // Formato completo se i dati ci sono
                : `N/D`; // Stringa pulita se i dati sono mancanti o parziali. Questo previene la regressione.
            ...
            finalForecast.push({
                ...
                // Usiamo la stringa 'currentDataString' che contiene la logica anti-regressione
