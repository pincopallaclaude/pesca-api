...
/**
 * @fileoverview IL DIRETTORE D'ORCHESTRA.
 * Coordinamento: Logica di fetching da più API, aggregazione dati, gestione cache e innesco del PescaScore.
 * NON contiene logica di business pura o I/O diretto, che sono delegati ai moduli dedicati.
 */
...
// --- MODIFICA (Aggiunta import P.H.A.N.T.O.M.) ---
const { triggerProactiveAnalysis, isAnalysisCached } = require('./services/proactive_analysis.service');
// --- FINE MODIFICA ---
...
async function getUnifiedForecastData(location) {
    ...
    // --- MODIFICA (Innesco P.H.A.N.T.O.M.) ---
    // Innesco P.H.A.N.T.O.M. (non-blocking) - Inizializza l'analisi AI in background
    if (!isAnalysisCached(cacheKey)) {
        // NON usiamo 'await' qui. Avviamo la funzione e proseguiamo immediatamente.
        triggerProactiveAnalysis(lat, lon, cacheKey); 
    }
    // --- FINE MODIFICA ---
