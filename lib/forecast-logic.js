 ...
 * Restituisce l'array con le previsioni giornaliere.
 * @param {string} location - Coordinata "lat,lon".
 * @returns {Promise<Array>} - Array di previsioni giornaliere.
 */
async function getUnifiedForecastData(location) {
    ...
    console.log(`[Master Fetch] Location normalizzata a 3 decimali: "${normalizedLocation}"`);
    ...
    if (cacheLocks.has(cacheKey)) {
        ...
        // poi estrae solo l'array forecast prima di restituirlo.
        return cacheLocks.get(cacheKey).then(fullResponse => fullResponse.forecast);
    }
    ...
    try {
        ...
        if (cachedData) {
            ...
            // Restituisce solo l'array forecast
        ...
        if (isPosillipo) {
            console.log(`[Master Fetch Log] Location is Posillipo. Adding premium fetch (Stormglass)...`);
        ...
        // 4. RESTITUISCE SOLO L'ARRAY DI FORECAST come richiesto.
        ...
        if (cachedData) {
            ...
            return cachedData.forecast; // Restituisce l'array forecast cachato
        }
