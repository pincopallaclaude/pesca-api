...
function processStandardData(wwoRawData, openMeteoData) {
    ...
    return {
        ...
        // SOLUZIONE: Insieme ai dati processati, restituiamo anche i dati GRENZZI di WWO
        processedForecast: processedForecast,
        wwoRawForecast: forecastData // Questo contiene i dati originali
    };
}
async function fetchAndProcessForecast(location) {
    ...
    if (location === POSILLIPO_COORDS) {
        ...
        try {
            ...
            // MANUALE MA NECESSARIO: attacchiamo anche qui i dati grezzi
            rawForecastData.wwoRawForecast = wwoResponse.data.data.weather;
        } catch (error) {
    ...
    // Ora il ciclo itera sull'array giusto 'processedForecast'
    for (let i = 0; i < rawForecastData.processedForecast.length; i++) {
        const dailyData = rawForecastData.processedForecast[i];
        // Recuperiamo i dati GRENZZI corrispondenti per l'accesso a 'winddir16Point'
        const originalWwoDaily = rawForecastData.wwoRawForecast[i];
        ...
        // Usiamo i dati grezzi per accedere a swellHeight_m
        const swellHeight_m = originalWwoDaily.hourly[4].swellHeight_m;
        ...
        finalForecast.push({
            ...
            trendPressione,
            // CORREZIONE DEFINITIVA: accediamo ai dati GRENZZI originali
...
// REINSERIAMO LA FUNZIONE MANCANTE
function calculatePescaScore(params) {
    ...
    if (prevWindSpeed > 30 && dailyWindSpeedKph < prevWindSpeed) {
        score += 2.0;
        reasons.push({ icon: 'wind', text: "Vento in calo", points: "+2.0", type: "positive" });
    } else if (dailyWindSpeedKph > 5 && dailyWindSpeedKph < 20) {
        score += 1.0;
        reasons.push({ icon: 'wind', text: "Vento ideale", points: "+1.0", type: "positive" });
    } else if (dailyWindSpeedKph > 30) {
        score -= 2.0;
        reasons.push({ icon: 'wind', text: "Vento troppo forte", points: "-2.0", type: "negative" });
    } else {
        reasons.push({ icon: 'wind', text: "Condizioni vento neutre", points: "+0.0", type: "neutral" });
    }
    if (isNewOrFullMoon) {
        score += 1.0;
        reasons.push({ icon: 'moon', text: "Fase lunare favorevole", points: "+1.0", type: "positive" });
    } else {
        reasons.push({ icon: 'moon', text: "Fase lunare neutra", points: "+0.0", type: "neutral" });
    }
    if (avgCloudCover > 60) {
        score += 1.0;
        reasons.push({ icon: 'clouds', text: "Cielo molto nuvoloso", points: "+1.0", type: "positive" });
        ...
        reasons.push({ icon: 'clouds', text: "Copertura nuvolosa neutra", points: "+0.0", type: "neutral" });
    }
    if (waveHeightMax !== null) {
        if (prevWaveHeightMax !== null && (waveHeightMax >= 1.25 && waveHeightMax < 2.5) && (prevWaveHeightMax >= 2.5)) {
            score += 2.0;
            reasons.push({ icon: 'waves', text: "Mare in scaduta", points: "+2.0", type: "positive" });
        } else if (waveHeightMax >= 0.5 && waveHeightMax < 1.25) {
            score += 2.0;
            reasons.push({ icon: 'waves', text: "Mare poco mosso (ideale)", points: "+2.0", type: "positive" });
        } else if (waveHeightMax >= 1.25 && waveHeightMax < 2.5) {
            score += 1.0;
            reasons.push({ icon: 'waves', text: "Mare mosso", points: "+1.0", type: "positive" });
        } else if (waveHeightMax < 0.5) {
            score -= 1.0;
            reasons.push({ icon: 'waves', text: "Mare calmo (negativo)", points: "-1.0", type: "negative" });
        } else if (waveHeightMax >= 2.5) {
            score -= 2.0;
            reasons.push({ icon: 'waves', text: "Mare troppo agitato", points: "-2.0", type: "negative" });
        } else {
            reasons.push({ icon: 'waves', text: "Stato del mare neutro", points: "+0.0", type: "neutral" });
        }
    } else {
        reasons.push({ icon: 'waves', text: "Stato del mare N/D", points: "+0.0", type: "neutral" });
    }
    if (waterTempAvg !== null) {
        if (waterTempAvg >= 12 && waterTempAvg <= 20) {
            score += 1.0;
            reasons.push({ icon: 'water_temp', text: "Temp. acqua ottimale", points: "+1.0", type: "positive" });
    ...
    if (currentVelocityAvg !== null) {
        if (prevCurrentVelocityAvg !== null) {
            ...
            if (currentDiff < -0.1) {
                score += 1.0;
                reasons.push({ icon: 'currents', text: "Correnti in calo", points: "+1.0", type: "positive" });
            } else if (currentDiff > 0.1) {
                score += 0.5;
                reasons.push({ icon: 'currents', text: "Correnti in aumento", points: "+0.5", type: "positive" });
            } else {
                reasons.push({ icon: 'currents', text: "Correnti stabili", points: "+0.0", type: "neutral" });
            }
        } else {
            reasons.push({ icon: 'currents', text: "Correnti stabili", points: "+0.0", type: "neutral" });
        }
        ...
        reasons.push({ icon: 'currents', text: "Correnti N/D", points: "+0.0", type: "neutral" });
    }
    return {
        numericScore: score,
