...
async function fetchOpenMeteoHourly(lat, lon) {
    const hourlyParams = ['temperature_2m','relative_humidity_2m','pressure_msl','cloud_cover','windspeed_10m','winddirection_10m','weathercode','wave_height','sea_surface_temperature','ocean_current_velocity'].join(',');
    ...
    for (let i = 0; i < apiData.time.length; i++) {
        ...
        dataByDay[date].push({
            ...
            currentVelocity: apiData.ocean_current_velocity ? apiData.ocean_current_velocity[i] : null,
        });
    }
    return dataByDay;
}
...
async function fetchAndProcessForecast(location) {
    const cacheKey = `forecast-data-v-final-${location}`; // Nuova versione per invalidare
    ...
    for (const wwoDay of wwoDailyData) {
        ...
        const hourlyScores = omHourly.map(h => {
            ...
            return {
                hour,
                score: calculateHourlyPescaScore({
                    pressure: h.pressure,
                    trendPressione,
