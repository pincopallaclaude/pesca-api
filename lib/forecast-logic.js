...
function processStormglassData(stormglassData, wwoAstroTideData) {
    ...
    finalForecast.push({
        ...
        tides: wwoDay.tides,
        hourly: hourlyTransformed, // Ora anche Stormglass ha i dati orari
    });
    });
...
async function fetchAndProcessForecast(location) {
    ...
    for (const dailyData of rawForecastData.forecast) {
        ...
        finalForecast.push({
            ...
            trendPressione,
            ventoDati: `${(dailyData.dailyWindSpeedKph / 1.852).toFixed(0)} kn ${dailyData.hourly[4]?.winddir16Point ?? ''}`,
