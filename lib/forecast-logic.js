...
const { triggerProactiveAnalysis } = require('./services/proactive_analysis.service.js');
...
async function getUnifiedForecastData(location) {
    ...
    try {
        ...
        // [P.H.A.N.T.O.M. - FASE 1] Innesco proattivo dell'analisi AI
        const avgScore = apiResponse.forecast[0]?.avgScore || 0;
        if (avgScore > 6.5 || avgScore < 3.5) {
            // Chiamata "Fire-and-forget": non usiamo await per non bloccare la risposta
            triggerProactiveAnalysis(apiResponse, normalizedLocation);
        }        
        return apiResponse;
    } catch (error) {
