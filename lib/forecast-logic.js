...
const utilities = {
    ...
    areCoordsNear: (coords1, coords2, toleranceKm = 1) => {
        ...
        // DEBUG: Logghiamo la distanza in km per verificare se si attiva la zona Posillipo
        log(`Distanza da Posillipo (${coords2}): ${distance.toFixed(3)} km. Tolleranza: ${toleranceKm} km.`, 'areCoordsNear DEBUG');
...
function calculateHourlyPescaScore(params, shouldLog = false) {
    ...
    let currentText = "Corrente N/D (Non Richiesto)"; // RIPRISTINATO IL MESSAGGIO STANDARD
...
async function getUnifiedForecastData(location) {
    ...
    try {
        ...
        if (isPosillipo) {
            ...
            // Gestiamo il fallimento del premium fetch separatamente
