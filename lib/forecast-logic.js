...
async function fetchStormglassData(lat, lon) {
    // PARAMETRI AGGIUNTIVI: currentSpeed, currentDirection
    ...
    console.log(`[ForecastLogic Log] Fetching Stormglass data for current at ${lat},${lon}`);
    ...
    for (const hourData of response.data.hours) {
        ...
        // Estrazione e conversione velocità corrente (m/s -> nodi)
        ...
        // Estrazione della direzione della corrente
    ...
    try {
        ...
        for (const wwoDay of wwoDailyData) {
            ...
            const enrichedHourlyData = omHourly.map(h => {
                ...
                const currentDirectionDeg = currentData.direction; 
