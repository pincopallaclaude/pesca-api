...
const { generateAndCacheAnalysis } = require('./services/proactive_analysis.service.js');
...
async function getUnifiedForecastData(location) {
    ...
    try {
        ...
        // --- P.H.A.N.T.O.M. Trigger ---
        // Calcola lo score medio e, se rilevante, avvia l'analisi AI in background.
        if (apiResponse.forecast && apiResponse.forecast.length > 0) {
            const avgScore = apiResponse.forecast[0].pescaScoreAggregato.numeric;
            if (avgScore > 6.5 || avgScore < 3.5) { // Condizione di trigger
                // Chiamata ASINCRONA: non usiamo 'await' per non bloccare la risposta principale.
                generateAndCacheAnalysis(apiResponse, normalizedLocation).catch(err => {
                    console.error('[Proactive AI Trigger] Chiamata in background fallita:', err.message);
                });
            }
        }
        return apiResponse;
