...
function calculateHourlyPescaScore(params) {
    ...
    // PRIMA MODIFICA: Aggiunto currentSpeedKn ai parametri destrutturati
    const { 
        pressure, trendPressione, windSpeedKph, isNewOrFullMoon, 
        cloudCover, waveHeight, waterTemp, currentSpeedKn // <-- NUOVO PARAMETRO
    } = params;
    
    // ==========================================
    // ========= NUOVO LOG DI DEBUG =============
    // Log dettagliato dei valori in ingresso per il calcolo del punteggio.
    console.log(`[Score Calc DEBUG] Ora: ${params.hour}, Vento: ${windSpeedKph.toFixed(1)} km/h, Onde: ${waveHeight?.toFixed(2)} m, Corrente: ${currentSpeedKn} kn, Temp Acqua: ${waterTemp}°C`);
    // ==========================================
    ...
    // --- INIZIO NUOVA LOGICA VENTO (Pesi variabili) ---
    if (windSpeedKph > 5 && windSpeedKph < 20) { 
        // Vento Moderato (5-20 km/h)
        if (waterTemp > 20) { // Acqua Calda (> 20°C): +1.5 (Premia l'ossigenazione)
            score += 1.5; reasons.push({ icon: 'wind', text: "Vento moderato (5-20 km/h) [Acqua Calda]", points: "+1.5", type: "positive" }); 
        } else { // Acqua Fredda (<= 20°C): +0.5 (Fattore neutro/leggermente positivo)
            score += 0.5; reasons.push({ icon: 'wind', text: "Vento moderato (5-20 km/h) [Acqua Fredda]", points: "+0.5", type: "neutral" });
        }
    } else if (windSpeedKph > 30) { 
        // Vento Forte (>30 km/h)
        score -= 2.0; reasons.push({ icon: 'wind', text: "Vento forte (>30 km/h) [Priorità Sicurezza]", points: "-2.0", type: "negative" }); 
    } else { 
        // Vento debole/variabile (<= 5 o tra 20 e 30)
        reasons.push({ icon: 'wind', text: "Vento debole/variabile", points: "+0.0", type: "neutral" }); 
    }
    // --- FINE NUOVA LOGICA VENTO ---
    ...
    // Onde (Wave Height) - Logica invariata
    ...
    // --- INIZIO NUOVA LOGICA TEMPERATURA ACQUA (Pesi differenziati) ---
    if (waterTemp !== null && waterTemp !== undefined) {
        if (waterTemp >= 14 && waterTemp <= 20) { 
            // Intervallo Ideale (14-20°C): +1.5
            score += 1.5; reasons.push({ icon: 'water_temp', text: "Temp. acqua ideale (14-20°C)", points: "+1.5", type: "positive" }); 
        } else if (waterTemp < 10) { 
            // Acqua Fredda (< 10°C): -1.5
            score -= 1.5; reasons.push({ icon: 'water_temp', text: "Acqua troppo fredda (<10°C)", points: "-1.5", type: "negative" }); 
        } else if (waterTemp > 23) {
            // Acqua Calda (> 23°C): -2.5
            score -= 2.5; reasons.push({ icon: 'water_temp', text: "Acqua troppo calda (>23°C)", points: "-2.5", type: "negative" }); 
        } else {
            // Range Intermedio/Neutro (10-14°C o 20-23°C): +0.0
            reasons.push({ icon: 'water_temp', text: "Temp. acqua intermedia/neutra", points: "+0.0", type: "neutral" }); 
        }
    } else {
        reasons.push({ icon: 'water_temp', text: "Temp. acqua N/D", points: "+0.0", type: "neutral" });
    }
    // --- FINE NUOVA LOGICA TEMPERATURA ACQUA ---
    
    // Correnti - Logica invariata
    let currentPoints = 0.0;
    let currentText = "Dati corrente non disp.";
    let currentType = "neutral";
    let currentIcon = "swap_horiz";

    if (currentSpeedKn !== null && currentSpeedKn !== 'N/D') {
    ...
    // ==========================================
    // ========= NUOVO LOG DI DEBUG =============
    console.log(`[Score Calc DEBUG] Score Corrente: ${currentPoints.toFixed(1)}, Score Totale: ${score.toFixed(1)}`);
    // ==========================================
...
async function fetchWwoDaily(lat, lon) {
    // Nota: Ho sostituito le variabili lat e lon con le versioni arrotondate (lat, lon)
    // usate anche per la Stormglass e OpenMeteo. La WWO API può funzionare con precisione variabile.
...
/**
 * Funzione helper per trovare il primo valore numerico disponibile in un oggetto dataPoint di Stormglass.
 * @param {object} dataPoint - Es. { "sg": 0.5, "dwd": 0.6 }
 * @returns {number|null} - Il primo valore numerico trovato o null.
 */
function findBestValue(dataPoint) {
    if (!dataPoint) return null;
    // Itera su tutte le chiavi (fonti come sg, dwd, meto)
    for (const source in dataPoint) {
        const value = dataPoint[source];
        // Controlla se il valore esiste e non è un numero (NaN)
...
async function fetchStormglassData(lat, lon) {
    // MODIFICA 1: Includiamo anche la direzione della corrente, è un dato fondamentale.
    ...
    for (const hourData of response.data.hours) {
        ...
        // MODIFICA 2: Utilizzo della funzione findBestValue per la velocità della corrente
        const rawSpeed = findBestValue(hourData.currentSpeed);
        let speedInKn = null; 

        if (rawSpeed != null) {
            speedInKn = rawSpeed * 1.94384; // m/s -> nodi
            // Correzione per valori molto piccoli, arrotondamento a 0.1 se > 0
            if (speedInKn > 0 && speedInKn < 0.1) speedInKn = 0.1;
        }
        
        // MODIFICA 3: Estrazione della direzione della corrente
        const rawDirection = findBestValue(hourData.currentDirection);
        let directionDeg = null;

        if (rawDirection != null) {
            directionDeg = rawDirection; // Gradi
        }
        
        dataByDay[date].push({
            hour: hourData.time.split('T')[1].split(':')[0], // Stringa HH
            // Salviamo come stringa con 1 decimale per consistenza con il formato "N/D"
            currentSpeedKn: speedInKn != null ? speedInKn.toFixed(1) : 'N/D', 
            // Salviamo la direzione in gradi per l'uso in gradiTo16PointDirection
            currentDirectionDeg: directionDeg != null ? directionDeg : null,
        });
    }
    return dataByDay;
}
...
async function fetchAndProcessForecast(location) {
    ...
    console.log(`[ForecastLogic Log] Location normalizzata a 3 decimali: "${normalizedLocation}"`);
    // ==========================================================
    ...
    if (cacheLocks.has(cacheKey)) {
        console.log(`[ForecastLogic Log] LOCK HIT: Chiave ${cacheKey} in fase di aggiornamento. Attendo...`);
        // Attende la risoluzione della Promise che ha acquisito il lock.
    ...
    try {
        ...
        if (cachedData) {
            console.log(`[ForecastLogic Log] Cache HIT for ${location}`);
            return cachedData;
        }

        console.log(`[ForecastLogic Log] Cache MISS for ${location}. Acquisizione LOCK e Fetching new data...`);
        ...
        if (isPosillipo) {
            console.log(`[ForecastLogic Log] Location is Posillipo. Adding premium fetch (Stormglass)...`);
            promises.push(fetchStormglassData(lat, lon).catch(err => {
                console.warn('[ForecastLogic Log] Stormglass fetch failed, proceeding without current data:', err.message);
        ...
        // ******************************************************************************
        // IL RESTO DELLA LOGICA DI ELABORAZIONE E ASSEMBLAGGIO DEI DATI RIMANE INVARIATA
        // ******************************************************************************
         ...
         * Ritorna un oggetto con velocità e direzione.
         * MODIFICA: La funzione ora restituisce anche la direzione della corrente.
         */
        const getStormglassCurrent = (date, hour) => {
            if (!isPosillipo || !stormglassData || !stormglassData[date]) {
                return { speed: 'N/D', direction: null };
            }
            const hourStr = String(hour).padStart(2, '0');
            const sgHourData = stormglassData[date].find(sg_h => sg_h.hour === hourStr);
            
            return sgHourData 
                ? { speed: sgHourData.currentSpeedKn, direction: sgHourData.currentDirectionDeg }
                : { speed: 'N/D', direction: null };
        };
        ...
        for (const wwoDay of wwoDailyData) {
            ...
            // TERZA MODIFICA (Punto 1): Uniamo i dati Open-Meteo e Stormglass
            const enrichedHourlyData = omHourly.map(h => {
                const currentHour = parseInt(h.time.split(':')[0], 10);
                // MODIFICA: Ora getStormglassCurrent restituisce un oggetto
                const currentData = getStormglassCurrent(date, currentHour); 
                const currentSpeedKn = currentData.speed;
                const currentDirectionDeg = currentData.direction; // Nuovo

                // ==========================================================
                // ========= NUOVO LOG DI DEBUG PER L'UNIONE DEI DATTI =======
                // ==========================================================
                console.log(`[Data Merge DEBUG] Giorno ${date} @ ${h.time}: Corrente Stormglass -> ${currentSpeedKn}`);
                // ==========================================================
                
                return { 
                    ...h, 
                    currentHour, 
                    currentSpeedKn, // Velocità unificata
                    currentDirectionDeg // Direzione unificata
                };
            });
            
            // TERZA MODIFICA (Punto 2): Calcolo dello score basato sui dati arricchiti
            const hourlyScoresData = enrichedHourlyData.map(h => {
                const scoreData = calculateHourlyPescaScore({
                    hour: h.currentHour, // Passiamo l'ora per il log di debug
                    ...
                    currentSpeedKn: h.currentSpeedKn // <-- Passiamo la corrente unificata
                });
            ...
            const findClosestTide = (hour, tides) => {
                if (!tides || tides.length === 0) return { type: 'N/A', tideTime: ''}; // Aggiunto tideTime
            ...
            // TERZA MODIFICA (Punto 3): Creazione del formato orario per il client basato sui dati arricchiti
            const hourlyClientFormat = enrichedHourlyData.map(h => {
                const currentHour = h.currentHour;
                ...
                return {
                    ...
                    // NUOVO: Direzione della Corrente
                    currentDirection: h.currentDirectionDeg != null ? degreesTo16PointDirection(h.currentDirectionDeg) : 'N/D', 
            ...
            const currentHourData = enrichedHourlyData.find(h => h.currentHour >= new Date().getHours()) ?? enrichedHourlyData[0];
            
            const temperaturaAcqua = String(Math.round(omHourly.map(h=>h.waterTemperature).reduce((a,b)=>a+b,0) / omHourly.length));
            
            // Troviamo il dato di corrente per l'ora rappresentativa
            const currentSpeedKn = currentHourData.currentSpeedKn;
            const currentDirectionDeg = currentHourData.currentDirectionDeg;
            const currentDirection = currentDirectionDeg != null ? degreesTo16PointDirection(currentDirectionDeg) : 'N/D';
            ...
            finalForecast.push({
                ...
                // Mare: stato mare (acronimo) + temp. acqua + velocità corrente (già in kn)
                // MODIFICA: Aggiungiamo anche la direzione della corrente nel riepilogo giornaliero
                mare: `${getSeaStateAcronym(currentHourData.waveHeight)} ${temperaturaAcqua}° | Corrente: ${currentSpeedKn} kn ${currentDirection}`,
        ...
        const apiResponse = {
            // MODIFICA: Aggiorniamo la stringa delle fonti
            fonti: "Open-Meteo & WorldWeatherOnline" + (isPosillipo ? " & Stormglass (Corrente Veloce + Direzione)" : ""),
        ...
        console.log(`[ForecastLogic Log] Cache aggiornata e LOCK rilasciato per: ${cacheKey}`);

        // 3. Rilascio del LOCK: Risolve la Promise per tutte le richieste in attesa.
        resolveLock(apiResponse);
        return apiResponse;
    } catch (error) {
        console.error(`[ForecastLogic Log] ERRORE durante l'aggiornamento forecast per ${cacheKey}: ${error.message}`, error.stack);
        
        // 4. Gestione del LOCK in caso di errore
        const cachedData = myCache.get(cacheKey);
        
        if (cacheLocks.has(cacheKey)) {
             // Il .catch() è per evitare UnhandledRejection in caso l'errore sia qui (es. resolveLock non definito)
             // e la promise sia ancora in attesa di essere risolta/rigettata altrove
             cacheLocks.get(cacheKey).catch(() => {}); 
             cacheLocks.delete(cacheKey);
             // Se siamo nel blocco `catch`, `rejectLock` dovrebbe essere definito
        ...
        if (cachedData) {
            console.warn(`[ForecastLogic Log] Fallback su dati cachati dopo errore per: ${cacheKey}`);
            return cachedData;
        }

        // Se non c'è cache e c'è errore, restituisce la struttura base per evitare crash frontend
        return {
            fonti: "Error",
            forecast: [],
            dateRange: "N/A"
        ...
        // Rimuove il lock se per qualche ragione non è stato rimosso prima (sicurezza)
        // Nota: se resolveLock/rejectLock sono stati chiamati, la promise è già settled.
        if (cacheLocks.has(cacheKey)) {
            cacheLocks.delete(cacheKey);
        }
    }
