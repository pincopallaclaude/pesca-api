...
const degreesTo16PointDirection = (deg) => {
    ...
    // Arrotondiamo a multipli di 22.5 gradi e prendiamo il modulo 16
...
function calculateHourlyPescaScore(params) {
    ...
    console.log(`[Score Calc DEBUG] Ora: ${params.hour}, Vento: ${windSpeedKph.toFixed(1)} km/h, Onde: ${waveHeight?.toFixed(2)} m, Corrente: ${currentSpeedKn} kn`);
    // ==========================================
    ...
    // Onde (Wave Height)
    if (waveHeight !== null && waveHeight !== undefined) { // Controllo più robusto
    ...
    // SECONDA MODIFICA: Nuova logica per la velocità della corrente
    // === NUOVA LOGICA PER LA VELOCITÀ DELLA CORRENTE ===
...
function findBestTimeWindow(hourlyScores, startHour, endHour) {
    ...
    for (let i = 0; i < relevantHours.length - 1; i++) {
        ...
        // L'array rilevante non è garantito che abbia indici consecutivi per le ore,
        // ma in questo caso, con Open-Meteo che fornisce tutte le ore, questa check è implicita.
        // Manteniamo la logica base, considerando la finestra di 2 ore consecutive filtrate.
        
        // Calcoliamo la media delle due ore consecutive
...
async function fetchAndProcessForecast(location) {
    ...
    // Variabili per gestire il resolve/reject della Promise di lock
    let resolveLock;
    let rejectLock;
    ...
    try {
        ...
        // 2. Scrittura in cache e rilascio del LOCK
        ...
        if (cacheLocks.has(cacheKey)) {
            // Rilascia il lock, risolvendo o rigettando la Promise per chi era in attesa (LOCK HIT)
            // Usiamo resolveLock/rejectLock definiti nel blocco try per manipolare la updatePromise.
            if (cachedData) {
                console.log(`[ForecastLogic Log] Errore di fetch ma cache presente. Servita cache vecchia a LOCK in attesa.`);
                resolveLock(cachedData); 
            } else {
                console.log(`[ForecastLogic Log] Errore di fetch e nessuna cache. Rigettata Promise a LOCK in attesa.`);
                rejectLock(error); 
            }
            cacheLocks.delete(cacheKey);
        }
        ...
        if (cachedData) {
            // Se c'è un dato vecchio, lo restituiamo al chiamante attuale come fallback.
            return cachedData;
        }

        // Se non c'è cache e il fetch fallisce, rilanciamo l'errore originale.
        throw error;
    }
}
...
module.exports = {
    ...
    // Esporto anche le utilità per eventuali test o usi esterni
