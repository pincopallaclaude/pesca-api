...
async function fetchAndProcessForecast(location) {
    ...
    for (const dailyData of rawForecastData.forecast) {
        ...
        const windDirection = dailyData.hourly[4]?.winddir16Point;
        console.log(`[DEBUG-3] Valore di winddir16Point per ${dailyData.date} al momento dell'uso:`, windDirection);
        
        // 2. Usiamo questa variabile per costruire la stringa, con un fallback robusto.
        finalForecast.push({
            ...
            trendPressione,
            ventoDati: `${(dailyData.dailyWindSpeedKph / 1.852).toFixed(0)} kn ${windDirection ?? ''}`,
