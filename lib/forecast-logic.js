...
function processStormglassData(stormglassData, wwoAstroTideData) {
    ...
    stormglassData.hours.forEach(hour => {
        ...
        dailyData[date].temps.push(hour.airTemperature.sg);
        dailyData[date].pressures.push(hour.pressure.sg);
        dailyData[date].humidities.push(hour.humidity.sg);
        dailyData[date].cloudCovers.push(hour.cloudCover.sg);
        dailyData[date].windSpeeds.push(hour.windSpeed.sg * 3.6);
        dailyData[date].waveHeights.push(hour.waveHeight.sg);
        dailyData[date].waterTemps.push(hour.waterTemperature.sg);
        dailyData[date].currentSpeeds.push(hour.currentSpeed.sg);
        if (hour.time.includes("T12:00")) {
            dailyData[date].windDirection = hour.windDirection.sg;
        }
    });
...
async function fetchStandardData(lat, lon) {
    ...
    const processedForecast = forecastData.map(dailyData => {
        ...
        // --- NUOVA LOGICA PER I DATI ORARI ---
        const hourlyTransformed = dailyData.hourly.map(h => ({
            time: formatTimeToHHMM(h.time), // Riformatta "600" in "06:00"
            tempC: h.tempC,
            weatherCode: h.weatherCode,
            weatherIconUrl: h.weatherIconUrl?.[0]?.value ?? null // Accesso sicuro all'URL
        }));
        ...
        return {
            ...
            // ... e sostituiscilo con la nostra nuova lista trasformata
