...
function calculateHourlyPescaScore(params) {
    ...
    // RIMOSSO: console.log(`[Score Calc DEBUG] Ora: ${params.hour}, Vento: ${windSpeedKph.toFixed(1)} km/h, Onde: ${waveHeight?.toFixed(2)} m, Corrente: ${currentSpeedKn} kn, Dir.Corrente: ${currentDirectionStr}`);
    ...
    // RIMOSSO: console.log(`[Score Calc DEBUG] Score Corrente: ${currentPoints.toFixed(1)}, Score Totale: ${score.toFixed(1)}`);
    ...
    try {
        ...
        for (const wwoDay of wwoDailyData) {
            ...
            const hourlyScoresData = omHourly.map(h => {
                ...
                // RIMOSSO: console.log(`[Data Merge DEBUG] Giorno ${date} @ ${h.time}: Corrente Stormglass -> ${currentSpeedKn} kn ${currentDirectionStr}`);
        ...
        console.error(`[Master Fetch Log] ERRORE durante l'aggiornamento forecast per ${cacheKey}: ${error.message}`, error.stack);
        ...
        if (cachedData) {
            console.warn(`[Master Fetch Log] Fallback su dati cachati dopo errore per: ${cacheKey}`);
