...
async function fetchOpenMeteoHourly(lat, lon) {
    ...
    for (let i = 0; i < forecastApiData.time.length; i++) {
        ...
        dataByDay[date].push({
            ...
            precipitationProbability: forecastApiData.precipitation_probability[i], // DATO NUOVO
            precipitation: forecastApiData.precipitation[i], // DATO NUOVO
    ...
    try {
        ...
        for (const wwoDay of wwoDailyData) {
            ...
            const hourlyClientFormat = omHourly.map(h => {
                ...
                // Logica per la corrente marina (solo Posillipo)
                let currentSpeedKn = null;
                if (isPosillipo && stormglassData && stormglassData[date]) {
                    const sgHourData = stormglassData[date].find(sg_h => parseInt(sg_h.hour, 10) === currentHour);
                    if (sgHourData && sgHourData.currentSpeed != null) {
                        let speedInKn = sgHourData.currentSpeed * 1.94384;
                        if (speedInKn > 0 && speedInKn < 0.1) speedInKn = 0.1;
                        currentSpeedKn = speedInKn; // Teniamo il numero, formattiamo nel frontend
                    }
                }
