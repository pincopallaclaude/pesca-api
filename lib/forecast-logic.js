        ...
        for (const wwoDay of wwoDailyData) {
            ...
            if (isPosillipo && stormglassData && stormglassData[date]) {
                ...
                if (sgHourData && sgHourData.currentSpeed != null) {
                    currentSpeedKn = (sgHourData.currentSpeed * 1.94384).toFixed(1);
                    console.log(`[Core] Using Stormglass current speed for ${date} @ ${currentHourString}:00: ${currentSpeedKn} kn`);
                }
            }
            ...
            finalForecast.push({
                ...
                mare: `${getSeaStateAcronym(currentHourData.waveHeight)} ${String(Math.round(omHourly.map(h=>h.waterTemperature).reduce((a,b)=>a+b,0) / omHourly.length))}° N/D kn`,
                ...
                // (e usati anche da altri moduli, es. MainHero)
