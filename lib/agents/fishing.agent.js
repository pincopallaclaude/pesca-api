...
  // === REACT LOOP ===
  for (let iteration = 0; iteration < MAX_ITERATIONS; iteration++) {
    ...
    try {
      // 🛑 MODIFICA CRITICA PER ISOLAMENTO DEL 503
      // Rimuoviamo i Tool solo nella prima iterazione (iteration === 0)
      // per vedere se la dimensione della loro descrizione JSON causa il 503.
      const toolsToSend = (iteration === 0) 
        ? [] 
      ...
      const candidate = await generateWithTools({
        ...
        tools: toolsToSend // Utilizza la lista di tool condizionale
      });
      
