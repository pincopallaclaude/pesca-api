...
  
  // Nuovi Controlli di sicurezza all'inizio per prevenire tutti i TypeError '0'
...
  
  // Context MINIMO per prompt iniziale
  const context = {
    location: {
      name: location.name,
      lat: location.lat,
...
    },
    summary: {
      // FIX: Uso di currentDayData per prevenire TypeError
      pescaScore: currentDayData.pescaScore.score.toFixed(1), 
      rating: currentDayData.pescaScore.rating,
      weather: currentDayData.weather || 'N/D',
      // Rimuoviamo i dettagli orari (wind, waves, temp_water) per snellire ulteriormente il prompt iniziale. 
      // L'agente li può recuperare chiamando analyze_weather_trend
      wind: 'Dati da recuperare con tool', 
      waves: 'Dati da recuperare con tool',
      temp_water: 'Dati da recuperare con tool'
    },
    // RIMOSSO tool_data: L'agente deve ora sapere che l'array dati è implicito se chiama analyze_weather_trend.
    // Questo è un rischio, ma è necessario per eliminare il 503.
    // L'agente ora assume che i dati orari (forecastData.hourly) siano disponibili
    // per il tool execution, ma non sono nel prompt.
    tool_data: {
      hourly_forecast: [] // Lasciamo un array vuoto come placeholder per il prompt, se la sintassi lo richiede.
    }
  };
  
  // Query SNELLA - i dettagli li recupera l'agent via tool
  const query = `Analizza le condizioni di pesca per ${location.name}.
  
PescaScore attuale: ${context.summary.pescaScore}/10 (${context.summary.rating})

USA I TOOL per recuperare:
- Dati storici zona (search_similar_episodes)
- Statistiche zona (get_zone_statistics)  
- Trend meteo 6h (analyze_weather_trend)
- Tecniche consigliate (search_knowledge_base)

Fornisci analisi dettagliata con raccomandazioni pratiche.`;
