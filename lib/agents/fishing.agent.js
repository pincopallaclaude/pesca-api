...
export async function generateProactiveAnalysis(forecastData, location) {
  ...
  // Controlli di sicurezza per l'array orario
  ...
  const context = {
    ...
    summary: {
      pescaScore: forecastData.daily[0].pescaScore.score.toFixed(1),
      rating: forecastData.daily[0].pescaScore.rating,
      weather: forecastData.daily[0].weather || 'N/D',
      // FIX: Uso di currentHourData e fallback a stringa vuota
      wind: `${currentHourData.wind || 'N/D'} km/h`, 
      waves: `${currentHourData.waveHeight || 'N/D'}m`,
      temp_water: `${currentHourData.waterTemp || 'N/D'}°C`
    },
    // AGGIUNTA: Array orario esplicitamente per l'uso dei tool (massimo 6 ore)
    tool_data: {
      // FIX: Slice solo se l'array esiste, altrimenti array vuoto
      hourly_forecast: hourlyData.slice(0, 6) 
    }
  };
