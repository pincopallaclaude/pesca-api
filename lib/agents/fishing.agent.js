...
IMPORTANTE: Sii specifico, usa dati concreti, e fornisci raccomandazioni pratiche.`;
  ...
  for (let iteration = 0; iteration < MAX_ITERATIONS; iteration++) {
    ...
    try {
      ...
      // Execute tool calls
      logger.debug(`[Agent] Executing ${toolCalls.length} tool(s)`);
...
export async function generateProactiveAnalysis(forecastData, location) {
  ...
  const context = {
    ...
    current_conditions: forecastData.hourlyData[0], // La chiave corretta è 'hourlyData'
    pescaScore: forecastData.pescaScoreData, // La chiave corretta è 'pescaScoreData'
    hourly_forecast: forecastData.hourlyData.slice(0, 6) // Usiamo 'hourlyData'
  };
  
  const query = `Genera un'analisi dettagliata delle condizioni di pesca per oggi in ${location.name}. 
  
ELEMENTI DA CONSIDERARE:
- PescaScore attuale: ${context.pescaScore.score.toFixed(1)}/10
