...
export async function loadModel() {
  try {
    ...
    try {
      ...
      logger.info('[ML] Model files found locally');
    } catch {
    ...
    logger.info('[ML] ✅ Feature scaler loaded');
...
export async function predictPescaScore(weatherData, astronomyData, location) {
  try {
    ...
    logger.debug(`[ML] Prediction: ${predicted.toFixed(2)} (${inferenceTime}ms)`);
