...
export async function findSimilarEpisodes(currentConditions, limit = 5) {
    ...
    try {
        ...
        // --- INIZIO NUOVA MODIFICA CRITICA (Gestione 400 Bad Request) ---
        // ChromaDB richiede che tutti i valori dell'embedding siano numeri finiti.
        const hasNonFinite = queryEmbedding.some(v => !Number.isFinite(v));
        
        if (hasNonFinite) {
            logger.warn('[Memory] Embedding vector contained non-finite values (NaN/Infinity). Replacing them with 0.');
            // Mappa l'array sostituendo i valori non finiti con 0
            queryEmbedding = queryEmbedding.map(v => Number.isFinite(v) ? v : 0);
        }
        // --- FINE NUOVA MODIFICA CRITICA ---
        
        const response = await axios.post(`${CHROMA_API_URL}/collections/fishing_episodes/query`, {
            query_embeddings: [queryEmbedding], // Uso il vettore pulito (queryEmbedding)
