...
/**
 * Hybrid Memory System (Zero-Cost) v2.0 - Con ChromaDB server via Axios
 */
...
import axios from 'axios'; // Usiamo Axios per l'interazione diretta con l'API REST di ChromaDB
...
const CHROMA_API_URL = 'http://127.0.0.1:8001/api/v1'; // URL DIRETTO ALL'API DEL SERVER CHROMADB
...
async function initChroma() {
    try {
        ...
        if (response.status !== 200) {
            throw new Error(`Heartbeat fallito, status: ${response.status}`);
        }
        logger.log(`[Memory] Connessione a ChromaDB v${response.data} riuscita.`);

        // 2. Creazione della collection 'fishing_episodes' se non esiste
        // NOTA: Non specifichiamo embedding_function, perché generiamo l'embedding localmente (Gemini)
        ...
        isChromaReady = true;
        logger.log('[Memory] ChromaDB pronto per ricevere richieste via API.');

    } catch (error) {
        logger.error('[Memory] ChromaDB init via Axios fallito:', error.message);
        throw error;
    }
}
...
export async function saveEpisode(data) {
  ...
  try {
    ...
    // Non è necessario l'import dinamico se è già in cima al file
    ...
    // 3. Insert in ChromaDB (semantic search) via Axios
    if (embedding && isChromaReady) {
      await axios.post(`${CHROMA_API_URL}/collections/fishing_episodes/add`, {
        ids: [embeddingId],
      ...
      logger.log(`[Memory] Episode saved: ${embeddingId}`);
    } else {
...
export async function findSimilarEpisodes(currentConditions, limit = 5) {
  ...
  try {
    ...
    if (!queryEmbedding || !isChromaReady) {
      logger.error('[Memory] Query embedding generation failed or Chroma not ready.');
      return [];
    }
    
    // Query a ChromaDB via Axios
    const response = await axios.post(`${CHROMA_API_URL}/collections/fishing_episodes/query`, {
        query_embeddings: [queryEmbedding],
        n_results: limit * 2,
        include: ["metadatas", "distances", "documents", "ids"] // Chiediamo tutti i dati necessari
    });
    ...
    // Preparazione per la query SQLite
    ...
    // Mappa la similarità (distance) ai risultati di SQLite
    const enrichedEpisodes = episodes.map((ep) => {
      const idxInSemantic = embeddingIds.indexOf(ep.embedding_id);
      return {
        ...ep,
        similarity: idxInSemantic !== -1 ? semanticResults.distances[0][idxInSemantic] : null,
...
export async function runCleanupPolicy() {
  ...
  try {
    ...
    // 1. Aggregazione dei dati vecchi (SQLite)
