...
export async function findSimilarEpisodes(currentConditions, limit = 5) {
    ...
    try {
        ...
        // --- MODIFICHE PER RISOLVERE 400 Bad Request ---
        // 1. Pulisce e appiattisce il vettore
        queryEmbedding = queryEmbedding.flat().map(v => Number.isFinite(v) ? v : 0);

        // 2. CRITICO: Controlla che il vettore non sia vuoto dopo la pulizia. 
        // Se è vuoto, c'è un errore nell'embedding che non possiamo risolvere qui, quindi abortiamo.
