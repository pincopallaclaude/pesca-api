...
export async function saveEpisode(data) {
    ...
    try {
        ...
        // CORREZIONE 1: Rafforza il check sull'embedding
        if (!embedding || !Array.isArray(embedding) || embedding.length === 0) {
            logger.warn('[Memory] Embedding generation failed (empty or invalid), skipping semantic storage.');
        }

        // 2. Insert in SQLite (structured data)
        const timestamp = Date.now(); // Variabile per timestamp uniforme
        ...
        const result = stmt.run(
            ...
            userFeedback || null,
            outcome || null,
            (embedding && Array.isArray(embedding) && embedding.length > 0) ? embeddingId : null,
            process.env.ML_MODEL_VERSION || '1.0'
        );
        
        // 3. Insert in ChromaDB (semantic search) - Solo se l'embedding è riuscito e Chroma è pronto
        // CORREZIONE 2: Rafforza la condizione d'inserimento
        if (isChromaReady && embedding && Array.isArray(embedding) && embedding.length > 0) { 
            const payload = {
                ids: [embeddingId],
            ...
            // Mantiene il logging dettagliato per debug futuri
        ...
        return { success: true, episodeId: result.lastInsertRowid, embeddingId: (embedding && Array.isArray(embedding) && embedding.length > 0) ? embeddingId : null };
    } catch (error) {
        // Log di errore di Axios per debug
