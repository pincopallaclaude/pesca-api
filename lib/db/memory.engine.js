...
// NOTA: La creazione di CHROMA_PATH non è più necessaria qui perché la modalità server la gestisce
...
// Crea un'istanza della nostra funzione di embedding personalizzata
const geminiEmbeddingFunction = {
  generate: async function (texts) {
    // Chiama la nostra funzione esistente da gemini.service.js
...
async function initChroma() {
  try {
    // Ci connettiamo al server ChromaDB che gira in background, avviato da start.sh
    const CHROMA_URL = 'http://localhost:8001';
    
    logger.log(`[Memory] Connessione al server ChromaDB a ${CHROMA_URL}...`);
    ...
    episodesCollection = await chromaClient.getOrCreateCollection({
      ...
      embeddingFunction: geminiEmbeddingFunction // SPECIFICHIAMO LA FUNZIONE DI EMBEDDING!
    });
    
    logger.log('[Memory] ChromaDB episodes collection initialized with Gemini embeddings.');
  } catch (error) {
