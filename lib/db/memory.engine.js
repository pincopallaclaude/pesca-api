...
export async function saveEpisode(data) {
    ...
    try {
        ...
        const result = stmt.run(
            sessionId,
            timestamp, // Usa la variabile timestamp
            location.lat,
            location.lon,
            location.name,
        ...
        if (embedding && isChromaReady) {
            const payload = {
                ...
                metadatas: [{ 
                    ...
                    created_at: timestamp // AGGIUNTO e uniformato il timestamp con SQLite
                }],
                documents: [episodeText]
            };

            // MODIFICA CRITICA: Serializza il payload per assicurare che venga stampato in caso di 400
            ...
            logger.warn(`[Memory] Salvataggio su ChromaDB saltato: embedding non generato o Chroma non pronto.`);
        ...
        // Log modificato per catturare il dettaglio 400 da Axios
