    ...
    try {
        ...
        if (embedding && isChromaReady) {
            const payload = {
                ...
                metadatas: [{ session_id: sessionId, location_name: location.name, pesca_score: pescaScore }],
                documents: [episodeText]
            };

            logger.debug('[Memory] ChromaDB Payload:', { 
                ids: payload.ids, 
            ...
            logger.log(`[Memory] Episodio salvato su ChromaDB via API: ${embeddingId}`);
             ...
             logger.warn(`[Memory] Episodio salvato solo su SQLite: ${result.lastInsertRowid}`); // Assicurati che 'result' sia definito prima
        }
