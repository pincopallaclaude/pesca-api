...
// better-sqlite3 è sincrono, quindi non è necessario wrappare l'oggetto Database.
...
let chromaCollectionId = null; // ID interno della collezione per query
...
async function initChroma() {
    try {
        ...
        // Recupera l'ID della collezione per usarlo nelle query (risolve InvalidUUID)
        const collectionResponse = await axios.get(`${CHROMA_API_URL}/collections?name=fishing_episodes`);
        if (collectionResponse.data && collectionResponse.data.length > 0) {
            chromaCollectionId = collectionResponse.data[0].id;
            logger.log(`[Memory] Acquisito ID collezione ChromaDB: ${chromaCollectionId}`);
        } else {
            throw new Error("Impossibile recuperare l'ID interno della collezione 'fishing_episodes'.");
        }
...
export async function saveEpisode(data) {
    ...
    try {
        ...
        if (isChromaReady && validEmbedding.length > 0) { 
            ...
            // L'endpoint è corretto per l'aggiunta ad una collezione
...
export async function findSimilarEpisodes(currentConditions, limit = 5) {
    ...
    try {
        // CRITICO: Usa ID UUID se disponibile, altrimenti fallisci
        if (!isChromaReady || !chromaCollectionId) {
            logger.warn("[Memory] ChromaDB non pronto o ID collezione non acquisito, impossibile eseguire ricerca semantica.");
            return [];
        }
        ...
        // Correzione: Pulisce e appiattisce il vettore, assicurando un array di numeri validi.
        queryEmbedding = queryEmbedding.flat().map(v => Number.isFinite(v) ? v : 0);

        if (queryEmbedding.length === 0) {
            logger.error('[Memory] Processed embedding is empty or invalid size after cleaning. Aborting ChromaDB query.');
            return [];
        }
        
        // ** NUOVO ENDPOINT: Usa l'ID UUID della collezione anziché il nome **
        const response = await axios.post(`${CHROMA_API_URL}/collections/${chromaCollectionId}/query`, {
        ...
        // Modificato il logging dell'errore per essere più chiaro
        logger.error('[Memory] Find similar episodes failed:', error.response ? `Request failed with status code ${error.response.status}. Full error: ${JSON.stringify(error.response.data)}` : error.message);
        return [];
    }
}

/**
 * Recupera statistiche aggregate per zona (Resa ASINCRONA per coerenza API)
 */
export async function getZoneStats(lat, lon, radius = 0.1) {
    // L'operazione sottostante di better-sqlite3 è sincrona, 
    // ma la funzione è avvolta in 'async' per allinearla alla chiamata 'await' nell'agente.
    try {
        const stmt = db.prepare(`
            SELECT 
        ...
        // Ritorno il risultato, che viene implicitamente risolto come Promise
        return stats;
    } catch (error) {
        logger.error('[Memory] getZoneStats failed:', error);
        throw error;
    }
}
...
export async function runCleanupPolicy() {
    ...
    try {
        ...
        if (embeddingIds.length > 0 && isChromaReady) {
            // Usa l'ID della collezione acquisito in initChroma
            await axios.post(`${CHROMA_API_URL}/collections/${chromaCollectionId}/delete`, { ids: embeddingIds });
