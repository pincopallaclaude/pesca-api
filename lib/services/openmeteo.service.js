...
async function fetchOpenMeteoData(lat, lon, forecastDays = 7) {
    ...
    const hourlyParams = [
        ...
        'weather_code', 'wave_height', 'sea_surface_temperature', 'ocean_current_velocity'
    ].join(',');
    ...
    for (let i = 0; i < apiData.time.length; i++) {
        ...
        dataByDay[date].push({
            ...
            weatherCode: apiData.weather_code[i],
            waveHeight: apiData.wave_height[i],
            waterTemperature: apiData.sea_surface_temperature[i],
            currentVelocity: apiData.ocean_current_velocity[i]
        });
    }
