...
async function _initialize() {
    try {
        ...
        // --- MODIFICA CHIAVE: Controlla solo lo status code ---
        const response = await axios.get(`${CHROMA_API_URL}/heartbeat`);
        
        if (response.status !== 200) {
            throw new Error(`Heartbeat fallito con status ${response.status}`);
        }
        logger.log(`[ChromaDB] ✅ Heartbeat ricevuto. Server locale alive.`);

        // Imposta lo stato come pronto (valore fittizio)
        _collection = { name: COLLECTION_NAME, count: 0 }; 

        // 2. Assicuriamo l'esistenza della Collection (tentativo GET, se 404, tentativo POST)
        try {
            const getResponse = await axios.get(`${CHROMA_API_URL}/collections/${COLLECTION_NAME}`);
