...
export async function rerankDocuments(query, documents, topK = 5) {
    ...
    try {
        ...
        console.log("--- DEBUG RERANKER: VALORE RESTITUITO ---");
        const finalResult = rankedDocs.slice(0, topK);
        console.log(JSON.stringify(finalResult.slice(0, 2), null, 2)); // Stampa i primi 2 risultati
        console.log("--- FINE DEBUG RERANKER ---");        
