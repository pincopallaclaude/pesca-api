...
async function queryKnowledgeBase(queryText, nResults = 5) {
    ...
    try {
        ...
        // 🔥 LOG DI ISPEZIONE VETTORI
        log(`[Vector Inspect] Query Vector (prime 5 dims): [${queryVector.slice(0, 5).map(v => v.toFixed(5)).join(', ')}]`);
        if (knowledgeBase.length > 0 && knowledgeBase[0].embedding) {
            log(`[Vector Inspect] KB Vector (prime 5 dims):     [${knowledgeBase[0].embedding.slice(0, 5).map(v => v.toFixed(5)).join(', ')}]`);
        }
        ...
        const cosineSimilarity = (vecA, vecB) => {
            ...
            // Aggiunto controllo di sicurezza per divisione per zero
