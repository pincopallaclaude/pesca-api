...
async function queryKnowledgeBase(query, topK = 5, filters = {}) {
    try {
        ...
        // ==========================================================
        const hybridResults = performHybridSearch(expandedQuery, candidateDocs, similarities);

        log(`[Vector Service] 📊 Top 5 risultati IBRIDI:`);
        hybridResults.slice(0, 5).forEach((s, i) => {
            log(`  ${i+1}. Score: ${s.similarity.toFixed(4)} | Sem: ${s._debug.semantic.toFixed(4)}, Key: ${s._debug.keyword.toFixed(4)} | Text: ${(s.text || '').substring(0, 60)}...`);
        });
