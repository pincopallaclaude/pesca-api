...
async function queryKnowledgeBase(query, topK = 5) {
    try {
        ...
        if (!knowledgeBase || knowledgeBase.length === 0) {
            log('[Vector Service] ⚠️ Knowledge Base vuota!');
            return [];
        }
        ...
        // Funzione di calcolo similarità (mantenuta interna per chiarezza)
        ...
        const similarities = knowledgeBase.map(doc => ({
            text: doc.content, // Usa 'content' come da struttura KB
        ...
        const results = similarities
            ...
            .slice(0, topK)
            // La funzione ora restituisce l'oggetto completo, con testo e similarità
...
export {
    embeddingModel,
    populateIndex,
    queryKnowledgeBase, // Funzione aggiornata
