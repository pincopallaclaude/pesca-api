    ...
    try {
        ...
        const results = knowledgeBase
            .map(doc => ({
                ...
                // Se doc.embedding fosse un array di numeri, questo potrebbe causare un errore, 
                // ma assumiamo che la struttura sia stata aggiornata per includere { values: [...] }.
                similarity: cosineSimilarity(queryVector, doc.embedding.values), 
            }))
            // MODIFICA: Filtra i risultati con una similarità inferiore alla soglia
        ...
        // Sostituito console.log con log
        log(`[VectorService] Found ${results.length} relevant documents via JS array search (filtered).`);
        return results;
