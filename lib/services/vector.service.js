...
function populateIndex(docs, embeddings) {
    ...
    knowledgeBase = docs.map((doc, index) => ({
        ...doc,
        // Nota: Qui memorizziamo l'array di numeri direttamente se proveniente da uno script esterno.
        // Se si usa l'oggetto completo dell'API, si dovrà adattare.
...
function loadKnowledgeBaseFromFile() {
    try {
        if (fs.existsSync(KB_FILE_PATH)) {
            ...
            // LOG DI DEBUG: Mostra la struttura del file caricato
            log('--- [VECTOR DEBUG] Struttura del knowledge_base.json caricato: ---');
            if (Array.isArray(parsedData)) {
                log(`Tipo: Array, Lunghezza: ${parsedData.length}`);
                if (parsedData.length > 0) log(`Struttura primo elemento: ${JSON.stringify(Object.keys(parsedData[0]))}`);
            } else if (typeof parsedData === 'object' && parsedData !== null) {
                log(`Tipo: Oggetto, Chiavi: ${JSON.stringify(Object.keys(parsedData))}`);
            } else {
                log(`Tipo: Sconosciuto`);
            }
            log('-------------------------------------------------------------');
            ...
            if (parsedData.docs && parsedData.embeddings && Array.isArray(parsedData.docs)) {
                ...
                // Gestisce la retrocompatibilità se il file è già nel formato interno (Array di oggetti con 'embedding')
                knowledgeBase = parsedData;
                log(`[VectorService] Knowledge base loaded directly (legacy internal format) from ${KB_FILE_PATH}. Total documents: ${knowledgeBase.length}`);
            } else {
                 throw new Error("Formato di knowledge_base.json non riconosciuto. Non è un array di documenti con embedding né un oggetto {docs, embeddings}.");
            }
        } else {
            // Sostituito console.warn con log
...
async function queryKnowledgeBase(queryText, nResults = 2) {
    ...
    try {
        ...
        // 2. Calcola la similarità per ogni documento
        const allSimilarities = knowledgeBase.map(doc => ({
            text: doc.content,
            // Accesso al vettore tramite .values per coerenza con l'output API
            similarity: cosineSimilarity(queryVector, doc.embedding.values),
        }));

        // LOG DIAGNOSTICO: Stampa i primi 5 punteggi di similarità per il debug
        log(`[VectorService] Raw similarities (Top 5): ${allSimilarities.slice(0, 5).map(s => s.similarity.toFixed(4)).join(', ')}`);

        // 3. Ordina e prendi i migliori risultati SENZA FILTRO AGGRESSIVO
        const results = allSimilarities
            .sort((a, b) => b.similarity - a.similarity) // Ordina per similarità decrescente
            .slice(0, nResults);
            
        // Sostituito console.log con log
        log(`[VectorService] Found ${results.length} relevant documents.`);
        return results;
