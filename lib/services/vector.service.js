    ...
    try {
        ...
        const cosineSimilarity = (vecA, vecB) => {
            if (!Array.isArray(vecA) || !Array.isArray(vecB) || vecA.length !== vecB.length) {
                return 0; // Se i vettori non sono validi, la similarità è zero
            }
        ...
        log(`[Vector Inspect] Inizio calcolo similarità per ${knowledgeBase.length} documenti...`);
        const allSimilarities = knowledgeBase.map((doc, index) => {
            // 🔥 LOG DI ISPEZIONE PER OGNI DOCUMENTO (Solo i primi 3 per non inondare i log)
            if (index < 3) { 
                log(`[Vector Inspect] Doc #${index}: Embedding valido? ${Array.isArray(doc.embedding)}`);
            }
            return {
                text: doc.content,
                source: doc.source, // Mantengo la source per chiarezza nel risultato finale
                similarity: cosineSimilarity(queryVector, doc.embedding)
            };
        });
        log(`[Vector Inspect] Calcolo similarità completato.`);

        allSimilarities.sort((a, b) => b.similarity - a.similarity);

        const threshold = 0.5; // Abbassata la soglia per catturare più risultati in RAG multi-query
