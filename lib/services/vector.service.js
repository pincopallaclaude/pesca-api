...
async function queryKnowledgeBase(query, topK = 5) {
    try {
        ...
        // 🔥 LOG DETTAGLIATO DELLE TOP 5 SIMILARITÀ (PRIMA DEL FILTRO)
        ...
        // ==========================================================
        // 🔥 NUOVA LOGICA: THRESHOLD ADATTIVA 🔥
        // ==========================================================
        const maxSimilarity = similarities[0]?.similarity || 0;
        let threshold;

        if (maxSimilarity >= 0.7) {
            threshold = 0.6; // Alta confidenza -> soglia restrittiva
        } else if (maxSimilarity >= 0.55) {
            threshold = 0.45; // Media confidenza -> soglia standard
        } else {
            threshold = 0.35; // Bassa confidenza -> soglia permissiva
        }
        
        // 🔥 LOG CRUCIALE: Logga la decisione della soglia adattiva
