    ...
    for (let attempt = 0; attempt < MAX_RETRIES; attempt++) {
        try {
            ...
            if (error.status === 503 || error.message?.includes('503')) {
                ...
                log(`[GeminiService] Tentativo ${attempt + 1}/${MAX_RETRIES} fallito (503). Riprovo in ${Math.round(waitTime / 1000)}s...`);
            ...
            // Sostituito console.error con log e formato richiesto
            log(`[GeminiService] ❌ ERRORE durante la generazione: ${error.message}`);
