...
// --- MODELLI ---
const TEXT_MODEL_NAME = "gemini-pro"; // La tua scelta
...
async function generateWithBestModel(request) {
    const MAX_RETRIES_GEMINI = 2; // Manteniamo 2 retry
    let lastError = null;

    // --- 1. TENTATIVO CON GEMINI (con retry per 503/429) ---
    for (let attempt = 0; attempt < MAX_RETRIES_GEMINI; attempt++) {
        try {
            ...
            if (isOverloaded) {
                // MESSAGGIO DI WARNING PER SOVRACCARICO
                logger.warn(`[Multi-Model] ⚠️ Gemini è sovraccarico (${error.message}). Riprovo. ${attempt + 1}/${MAX_RETRIES_GEMINI}`);
                // CONTINUIAMO IL LOOP. NON USIAMO break; qui.
            } else {
                // ERRORE NON 503/429: Passa al fallback se non è l'ultimo tentativo.
                logger.error(`[Multi-Model] Errore non recuperabile con Gemini: ${error.message}`);
            }

            // LOGICA DI RETRY (Applicata se non è l'ultimo tentativo)
            if (attempt < MAX_RETRIES_GEMINI - 1) {
                const waitTime = Math.pow(2, attempt) * 1000;
                logger.warn(`[Multi-Model] Tentativo fallito. Attendo ${waitTime / 1000}s prima di riprovare...`);
                await delay(waitTime);
            } else {
                // ULTIMO TENTATIVO FALLITO: Passiamo al fallback
                logger.warn(`[Multi-Model] ${isOverloaded ? 'Sovraccarico persistente.' : 'Errore non recuperabile.'} Passaggio al fallback.`);
                break; 
            }
        }
    }
