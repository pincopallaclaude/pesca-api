...
// *** MODIFICA EFFETTUATA QUI: RIDOTTO A 2 TENTATIVI PER EVITARE TIMEOUT LUNGHI SU RENDER ***
const MAX_RETRIES_GEMINI = 2; 
...
async function generateWithBestModel(request) {
    ...
    const mistralStartTime = Date.Date.now();
    ...
    try {
        ...
        // Simula la struttura di risposta di Gemini (necessaria per la compatibilità con generateWithTools)
        const mistralGeminiFormat = {
            candidates: [{
                content: { parts: [{ text: mistralResponse.text }] }, // Assumendo che Mistral restituisca { text: '...' }
                finishReason: 'STOP',
                safetyRatings: [],
                citationMetadata: mistralResponse.citationMetadata || { groundingAttributions: [] } 
            }],
            usageMetadata: mistralResponse.usageMetadata || {},
            text: () => mistralResponse.text // Funzione di utilità
        };
        
        return { response: mistralGeminiFormat, provider: 'mistral' };
    ...
    try {
        ...
        // Simula la struttura di risposta di Gemini (necessaria per la compatibilità con generateWithTools)
        const claudeGeminiFormat = {
            candidates: [{
                content: { parts: [{ text: claudeResponse.text }] }, 
                finishReason: 'STOP',
                safetyRatings: [],
                citationMetadata: claudeResponse.citationMetadata || { groundingAttributions: [] } 
            }],
            usageMetadata: claudeResponse.usageMetadata || {},
            text: () => claudeResponse.text 
        };
        
        return { response: claudeGeminiFormat, provider: 'anthropic' };
    } catch (claudeError) {
...
export async function generateWithTools(agentRequest) {
    ...
    // Restituisce la struttura candidata formattata, necessaria per la gestione dei tool.
