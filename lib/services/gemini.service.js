...
async function generateWithBestModel(request) {
    const MAX_RETRIES_GEMINI = 3; // Aumentiamo i retry per la stabilità
    let lastError = null;
    ...
    for (let attempt = 0; attempt < MAX_RETRIES_GEMINI; attempt++) {
        try {
            ...
            if (isOverloaded && attempt < MAX_RETRIES_GEMINI - 1) {
                ...
                logger.warn(`[Multi-Model] ⚠️ Gemini è sovraccarico (${error.message}). Riprovo in ${waitTime / 1000}s...`);
                await delay(waitTime);
                // Continua il loop
            } else {
                // Fallimento non recuperabile (es. 400, 403) o ultimo tentativo fallito
                logger.warn(`[Multi-Model] Fallimento finale Gemini (${error.message}). Inizio fallback.`);
