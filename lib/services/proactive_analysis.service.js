...
async function runProactiveAnalysis(forecastData, locationKey) {
    ...
    try {
        ...
        // 1. CHIAMATA AL TOOL MCP (ora usa il router multi-model)
        const result = await mcpClient.callTool('analyze_with_best_model', {
            weatherData: firstDayForecast,
            location: locationForTitle
        });
        
        console.log(`[Proactive-AI] ✅ Analisi generata via ${result.metadata?.modelUsed || 'AI'}`);
        ...
        const finalAnalysisText = result.content[0].text;
        
        // =================================================================
        // LOG DIAGNOSTICO: ISPEZIONA LA RISPOSTA GREZZA DELL'AI
        console.log("--- INIZIO RISPOSTA GREZZA AI (PROACTIVE) ---");
        console.log(finalAnalysisText);
        console.log("--- FINE RISPOSTA GREZZA AI (PROACTIVE) ---");
        // =================================================================
        
        const timingMs = result.metadata?.timingMs || 'N/D';

        // 2. CACHING DEL RISULTATO (MODIFICATO PER INCLUDERE I METADATI)
        if (finalAnalysisText && finalAnalysisText.trim().length > 50) {
            const cacheKey = `analysis-v2-${locationKey}`;
            
            // CREA IL PAYLOAD COMPLETO DA SALVARE
            const analysisPayload = {
                analysis: finalAnalysisText.trim(), // Il testo dell'analisi in Markdown
                metadata: result.metadata,          // L'oggetto metadati completo
            };

            // SALVATAGGIO IN CACHE
            analysisCache.set(cacheKey, analysisPayload);
            
            console.log(`[Proactive-AI] ✅ Analisi per ${locationForTitle} generata e cachata con successo, inclusi metadati (${timingMs}ms).`);
        } else {
