...
async function runProactiveAnalysis(forecastData, locationKey) {
    ...
    const readableLocationName = forecastData?.forecast?.[0]?.location?.name || locationKey;
    console.log(`[Proactive-AI] Avvio analisi via MCP in background per ${readableLocationName}...`);
    ...
    try {
        ...
        if (finalAnalysis && finalAnalysis.trim().length > 50) {
            ...
            console.log(`[Proactive-AI] ✅ Analisi per ${readableLocationName} generata e cachata con successo (${timingMs}ms).`);
        } else {
            console.warn(`[Proactive-AI] ⚠️ Analisi generata per ${readableLocationName} vuota o troppo corta. Non cachata.`);
        }
