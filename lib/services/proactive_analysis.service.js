...
export async function runProactiveAnalysis(fullForecastData, normalizedLocation) {
  try {
    ...
    console.log(`[Proactive-AI DEBUG] Sto per chiamare il tool: '${toolToCall}'`);
    // === STEP 2: Generazione con Multi-Model via MCP ===
    const result = await mcpClient.callTool('analyze_with_best_model', { // La chiamata al tool è corretta.
